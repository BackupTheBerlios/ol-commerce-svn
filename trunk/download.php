<?php
/* -----------------------------------------------------------------------------------------
   $Id: download.php,v 1.1.1.1.2.1 2007/04/08 07:16:14 gswkaiser Exp $   
...
(c) 2004      XT - Commerce; www.xt-commerce.com
  ...
  require_once(DIR_FS_INC.'olc_random_name.inc.php');
  require_once(DIR_FS_INC.'olc_unlink_temp_dir.inc.php');
  ...
  if (!olc_db_num_rows($downloads_query)) die;
  $downloads = olc_db_fetch_array($downloads_query);
  ...
  olc_db_query(SQL_UPDATE . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " set download_count = download_count-1 where orders_products_download_id = '" . (int)$_GET['id'] . APOS);
  ...
  if (DOWNLOAD_BY_REDIRECT == TRUE_STRING_S) {
    // This will work only on Unix/Linux hosts
    olc_unlink_temp_dir(DIR_FS_DOWNLOAD_PUBLIC);
    $tempdir = olc_random_name();
    umask(0000);
    mkdir(DIR_FS_DOWNLOAD_PUBLIC . $tempdir, 0777);
    symlink(DIR_FS_DOWNLOAD . $downloads['orders_products_filename'], DIR_FS_DOWNLOAD_PUBLIC . $tempdir . '/' . $downloads['orders_products_filename']);
    olc_redirect(DIR_WS_DOWNLOAD_PUBLIC . $tempdir . '/' . $downloads['orders_products_filename']);
  } else {
