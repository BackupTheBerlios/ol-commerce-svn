<?php
/* --------------------------------------------------------------
$Id: whos_online_live.php,v 1.1.1.1.2.1 2007/04/08 07:16:33 gswkaiser Exp $
...
$application_name="Who is online (Live)";
require('includes/application_top.php');

$current_time=time();
$delete=DELETE_FROM . TABLE_WHOS_ONLINE. ' WHERE ';
if ($is_periodic)
{
...
$col_end = HTML_NBSP.'</td>';
$rab='>'.HTML_NBSP;
$align_right = ' align="right"'.$rab;
$align_center = ' align="center"'.$rab;
$col_start_center=$col_start.$align_center;
if ($is_periodic)
{
						    ...
						    <td valign="top">
						      <table border="0" width="100%" cellspacing="0" cellpadding="2">
						        <tr class="dataTableHeadingRow">'.
						          $col_start_center.TABLE_HEADING_CUSTOMERS.$col_end .
											$col_start_center."N°" .$col_end .
											$col_start_center.TABLE_HEADING_ORDER_TOTAL.$col_end .
											$col_start_center.TABLE_HEADING_DATE_PURCHASED.$col_end .
											$col_start_center.TABLE_HEADING_STATUS.$col_end .
											$col_start_center.TABLE_HEADING_ACTION.$col_end .'
						        </tr>
';
...
';
		if ($order_total_total>0)
		{
			...
			$orders_html.='
						        <tr class="dataTableHeadingRow">'.
						          $col_start.' colspan="6"'.$rab.$orders_html_line.$col_end .'
						        </tr>
';
		}
		$orders_html.='
					          ...
					          <tr>
					            <td valign="top">
						            <table border="0" width="100%" cellspacing="0" cellpadding="2">
						              <tr class="dataTableHeadingRow">
														'.
														$col_start_center.TABLE_HEADING_ONLINE.$col_end.
														$col_start_center.TABLE_HEADING_CUSTOMER_ID.$col_end.
														$col_start_center.TABLE_HEADING_FULL_NAME.$col_end.
														$col_start_center.TABLE_HEADING_IP_ADDRESS.$col_end.
														$col_start_center.TABLE_HEADING_ENTRY_TIME.$col_end.
														$col_start_center.TABLE_HEADING_LAST_CLICK.$col_end.
														$col_start_center.TABLE_HEADING_LAST_PAGE_URL.$col_end.'
						              </tr>
';
$whos_online_end_row='
					          	    <tr>
...
if ($visitors>0)
{
	include_once(DIR_FS_INC.'olc_get_short_description.inc.php');
	define('PRODUCTS_ID','products_id');
	define('PRODUCTS_MODEL','products_model');
	$cPath='cPath';
	$equal='=';
	$lparen='(';
	$lparen_long=HTML_NBSP.$lparen;
	$times=HTML_NBSP.'x'.HTML_NBSP;
	...
	$product_link=
	HTML_A_START.olc_href_link(ADMIN_PATH_PREFIX.'product_info.php','products_id=#&pop_up=true').
	'" title="'.TILDE.'" target="_blank">@'.HTML_A_END;
	$whos_online_row0='
										onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'"
...
*/
	$whos_online_row1='
					              		<tr id="#" style="display:table-row" class="dataTableRow"';
	$selected=false;
	...
	while ($whos_online = olc_db_fetch_array($whos_online_query))
	{
		...
		/*
		$pos=strpos($ip_address,$lparen);
		if ($pos!==false)
		{
			$ip_address_index=str_replace(DOT,UNDERSCORE,rtrim(substr($ip_address,0,$pos)));
		}
		else
		{
 ...
 ';
		...
		$whos_online_row.=
		$td_start.$align_right.gmdate($date_format, $time_online).$td_end.
		$td_start.$align_right.$customer_id.$td_end.
		$td_start.$rab.$full_name.$td_end.
		$td_start.$align_right.$ip_address.$td_end.
		$td_start.$align_right.date($date_format, $whos_online['time_entry']).$td_end.
...
';
		...
		if ($info)
		{
			...
			if ($length)
			{
				...
				(strpos($session_data, ';', $start_currency) - $start_currency + 1));
				$session_data_country = substr($session_data, $start_country,
				(strpos($session_data, ';', $start_country) - $start_country + 1));
				...
				if (is_object($cart))
				{
					...
					if ($n > 1)
					{
						$cart_content.=HTML_HR.TEXT_SHOPPING_CART_SUBTOTAL . BLANK .
						olc_format_price(abs($cart->show_total()),true,true);
					}
				}
				if ($cart_content)
				{
					$main_content.= '
	              						<tr>
		              						<td class="dataTableRowSelected" style="font-size:12px" valign="top">
		              							<b>' .TABLE_HEADING_SHOPPING_CART . '</b>
		              						</td>
		              						<td colspan="6">
	              								<table border="0">
				              						<tr>
						            						<td class="errorText" valign="top">
	'.
					$cart_content.
	'
	  	  						      					</td>
...
$main_content.=str_replace(HASH,HTML_HR,$whos_online_end_row);
if ($cart_total>0)
{
	$main_content.=
	str_replace(HASH,
	'<font color ="Red">'.
		TEXT_VALUE_OF_CARTS.olc_format_price($cart_total,true,true),$whos_online_end_row).'
	</font>'.HTML_BR.HTML_BR;
}
...
';
	...
	else
	{
		$sounds=EMPTY_STRING;
		$embed_sound='
		<EMBED SRC="'.ADMIN_PATH_PREFIX.'live_data/#" height="0" width="0" HIDDEN="true" AUTOSTART="true" style="display:none">';
...
For details see: http://www.capitalhead.com/1240.aspx
IE Hotfix: http://www.capitalhead.com/articles/click-to-activate-and-use-this-control---kb912812.aspx
<script type="text/javascript" src="'.DIR_WS_INCLUDES.'activate_ie_objects.js"></script>
-->
...
for (var i = 0; i < objects_count; i++)
{
    ...
    if (strExec.indexOf(activate_ie_objects_text)!=-1)
    {
	    try
	    {
	      eval(strExec);
