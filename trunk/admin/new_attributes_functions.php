<?php
/* --------------------------------------------------------------
   $Id: new_attributes_functions.php,v 1.1.1.1.2.1 2007/04/08 07:16:29 gswkaiser Exp $   
...
(c) 2004      XT - Commerce; www.xt-commerce.com
  ...
  function checkAttribute($current_value_id, $current_pid, $current_product_option_id) {
    ...
    $query = "SELECT * FROM " . TABLE_PRODUCTS_ATTRIBUTES . " where options_values_id = '" . $current_value_id . 
    	"' AND products_id = ' " . $current_pid . "' AND options_id = '" . $current_product_option_id . APOS;
  ...
  function getSortCopyValues($current_value_id, $current_pid) {
    ...
    if ($isSelected) {
      $query = "SELECT * FROM " . TABLE_PRODUCTS_ATTRIBUTES . "  where options_values_id = '" . $current_value_id . "' AND products_id = '" . $current_pid . APOS;
      $result = olc_db_query($query);
      while ($line = olc_db_fetch_array($result, olc_db_ASSOC)) {
