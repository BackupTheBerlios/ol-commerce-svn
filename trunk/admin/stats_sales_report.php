<?php
/* --------------------------------------------------------------
$Id: stats_sales_report.php,v 1.1.1.1.2.1 2007/04/08 07:16:32 gswkaiser Exp $
...
if ( ($_GET['report']) && (olc_not_null($_GET['report'])) )
{    $srView = $_GET['report'];
}
...
// detail
if ( ($_GET['detail']) && (olc_not_null($_GET['detail'])) )
...
// report views (1: yearly 2: monthly 3: weekly 4: daily)
if ( ($_GET['export']) && (olc_not_null($_GET['export'])) )
{    $srExp = $_GET['export'];
}
if ($srExp < 0 || $srExp > 2) {
	$srExp = $srDefaultExp;
...
// item_level
if ( ($_GET['max']) && (olc_not_null($_GET['max'])) ) {
	$srMax = $_GET['max'];
}
if (!is_numeric($srMax)) {
	$srMax = 0;
...
// order status
if ( ($_GET['status']) && (olc_not_null($_GET['status'])) )
...
// sort
if ( ($_GET['sort']) && (olc_not_null($_GET['sort'])) ) {
	$srSort = $_GET['sort'];
}
...
$startDate = "";
$startDateG = 0;
if ( ($_GET['startD']) && (olc_not_null($_GET['startD'])) )
{    $sDay = $_GET['startD'];
$startDateG = 1;
} else {
	$sDay = 1;
}
if ( ($_GET['startM']) && (olc_not_null($_GET['startM'])) )
{    $sMon = $_GET['startM'];
$startDateG = 1;
} else {
	$sMon = 1;
}
if ( ($_GET['startY']) && (olc_not_null($_GET['startY'])) )
{    $sYear = $_GET['startY'];
$startDateG = 1;
} else {
	$sYear = date("Y");
...
$endDate = "";
$endDateG = 0;
if ( ($_GET['endD']) && (olc_not_null($_GET['endD'])) ) {
	$eDay = $_GET['endD'];
	$endDateG = 1;
} else {
	$eDay = 1;
}
if ( ($_GET['endM']) && (olc_not_null($_GET['endM'])) ) {
	$eMon = $_GET['endM'];
	$endDateG = 1;
} else {
	$eMon = 1;
}
if ( ($_GET['endY']) && (olc_not_null($_GET['endY'])) ) {
	$eYear = $_GET['endY'];
	$endDateG = 1;
} else {
	$eYear = date("Y");
...
?>
        <tr>
          <td colspan="2">
					  <?php
						echo olc_draw_form('stat_sales_report', CURRENT_SCRIPT, EMPTY_STRING, 'get');
...
for ($i = 1; $i < 32; $i++) {
?>
                        <option<?php if ($j == $i) echo HTML_SELECTED; ?>><?php echo $i; ?></option>
<?php
}
?>
                    </select>
...
for ($i = 1; $i < 13; $i++) {
?>
                      <option<?php if ($m == $i) echo HTML_SELECTED; ?> value="<?php echo $i; ?>"><?php echo strftime("%B", mktime(0, 0, 0, $i, 1)); ?></option>
<?php
}
?>
                    </select>
...
for ($i = 10; $i >= 0; $i--) {
?>
                      <option<?php if ($y == $i) echo HTML_SELECTED; ?>><?php echo date("Y") - $i; ?></option>
<?php
}
?>
                    </select>
                  ...
                  <td rowspan="2" align="left" class="menuBoxHeading">
                    ...
                    <select name="detail" size="1">
                      <option value="0"<?php if ($srDetail == 0) echo HTML_SELECTED; ?>><?php echo DET_HEAD_ONLY; ?></option>
                      <option value="1"<?php if ($srDetail == 1) echo HTML_SELECTED; ?>><?php echo DET_DETAIL; ?></option>
                      <option value="2"<?php if ($srDetail == 2) echo HTML_SELECTED; ?>><?php echo DET_DETAIL_ONLY; ?></option>
                    </select><br/>
<?php echo REPORT_MAX; ?><br/>
                    <select name="max" size="1">
                      <option value="0"><?php echo REPORT_ALL; ?></option>
                      <option<?php if ($srMax == 1) echo HTML_SELECTED; ?>>1</option>
                      <option<?php if ($srMax == 3) echo HTML_SELECTED; ?>>3</option>
                      <option<?php if ($srMax == 5) echo HTML_SELECTED; ?>>5</option>
                      <option<?php if ($srMax == 10) echo HTML_SELECTED; ?>>10</option>
                      <option<?php if ($srMax == 25) echo HTML_SELECTED; ?>>25</option>
                      <option<?php if ($srMax == 50) echo HTML_SELECTED; ?>>50</option>
                    </select>
                  </td>
                  <td rowspan="2" align="left" class="menuBoxHeading">
...
?>
                      <option value="<?php echo $value["orders_status_id"]?>"<?php if ($srStatus == $value["orders_status_id"]) echo HTML_SELECTED; ?>><?php echo $value["orders_status_name"] ; ?></option>
<?php
}
?>
                    </select><br/>
                  ...
                  <td rowspan="2" align="left" class="menuBoxHeading">
                    ...
                    <select name="sort" size="1">
                      <option value="0"<?php if ($srSort == 0) echo HTML_SELECTED; ?>><?php echo SORT_VAL0; ?></option>
                      <option value="1"<?php if ($srSort == 1) echo HTML_SELECTED; ?>><?php echo SORT_VAL1; ?></option>
                      <option value="2"<?php if ($srSort == 2) echo HTML_SELECTED; ?>><?php echo SORT_VAL2; ?></option>
                      <option value="3"<?php if ($srSort == 3) echo HTML_SELECTED; ?>><?php echo SORT_VAL3; ?></option>
                      <option value="4"<?php if ($srSort == 4) echo HTML_SELECTED; ?>><?php echo SORT_VAL4; ?></option>
                      <option value="5"<?php if ($srSort == 5) echo HTML_SELECTED; ?>><?php echo SORT_VAL5; ?></option>
                      <option value="6"<?php if ($srSort == 6) echo HTML_SELECTED; ?>><?php echo SORT_VAL6; ?></option>
                    </select><br/>
                  </td>
                </tr>
                <tr>
...
for ($i = 10; $i >= 0; $i--) {
?>
                      <option<?php if ($y == $i) echo HTML_SELECTED; ?>><?php echo
                      date("Y") - $i; ?></option><?php
}
?>
                    </select>
                ...
                <tr>
                  ...
                  <?php echo olc_image_submit('button_update.gif', IMAGE_BUTTON_UPDATE_CART); ?>
                  </td>
              </table>
            </form>
          </td>
...
switch ($srView) {
	case '3':
?>
                      <td class="dataTableContent" align="right"><?php echo olc_date_long(date("Y-m-d\ H:i:s", $sr->showDate)) . " - " . olc_date_short(date("Y-m-d\ H:i:s", $sr->showDateEnd)); ?></td>
<?php
break;
case '4':
?>
                      <td class="dataTableContent" align="right"><?php echo olc_date_long(date("Y-m-d\ H:i:s", $sr->showDate)); ?></td>
<?php
break;
default;
?>
                      <td class="dataTableContent" align="right"><?php echo olc_date_short(date("Y-m-d\ H:i:s", $sr->showDate)) . " - " . olc_date_short(date("Y-m-d\ H:i:s", $sr->showDateEnd)); ?></td>
<?php
}
?>
...
?>
                    ...
                    <td class="dataTableContent" align="left"><?php echo HTML_A_START.olc_catalog_href_link("product_info.php?products_id=" . $info[$i]['pid']) ?>" target="_blank"><?php echo $info[$i]['pname']; ?></a>
<?php
if (is_array($info[$i]['attr'])) {
	...
	foreach ($attr_info as $attr) {
		...
		if ($price != 0) {
			...
			echo $currencies->format($price). RPAREN;
		}
		echo '</div>';
	}
}
