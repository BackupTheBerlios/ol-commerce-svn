...
//debug_stop();
		if (http_request)
		{
			...
			if (request_period>0)
			{
				...
				whos_online_timer=window.setInterval("periodic_request()", request_period);
				periodic_request();
			}
		}
		else
...
function periodic_request()
{
	if (!request_active)
	{
		make_AJAX_Request(calling_url,false,periodic,ajax_get);	//Make AJAX request
	}
}

function confirm_exit()
{
	return "Wollen Sie \'"+application_name+"\' wirklich beenden?";
}
