<?php
/* -----------------------------------------------------------------------------------------
   $Id: preistrend.php,v 1.1.1.1 2006/12/22 13:39:13 gswkaiser Exp $
...
define('MODULE_PREISTREND_FILE_TITLE' , '<hr noshade="noshade"/>Dateiname');
...
// include needed functions
  ...
  class preistrend {
    ...
    function preistrend() {
      ...
      $this->enabled = ((MODULE_PREISTREND_STATUS == TRUE_STRING_S) ? true : false);
    ...
    return array('text' =>  EXPORT_STATUS_TYPE.HTML_BR.
                            ...
                            HTML_A_START . olc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=preistrend') . '">' .
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = olc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PREISTREND_STATUS'");
    ...
    function install() {
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " ( configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PREISTREND_FILE', 'preistrend.txt',  '6', '1', '', now())");
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " ( configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PREISTREND_STATUS', 'true',  '6', '1', 'olc_cfg_select_option(array(\'True\', \'False\'), ', now())");
}
