<?php
/* -----------------------------------------------------------------------------------------
   $Id: milando.php,v 1.1.1.1 2006/12/22 13:39:11 gswkaiser Exp $
...
define('MODULE_MILANDO_TEXT_DESCRIPTION', 'Export - Milando (; getrennt)<br/><b>Format:</b><br/>Produktbezeichnung;Beschreibung;Preis;Url;Bild;Währung;Kategorie; Gültig bis;Gültig ab;FSK;Typ;Produkt id;Hersteller; Bestellnummer;Suchbegriffe;Preistyp');
define('MODULE_MILANDO_TEXT_TITLE', 'Milando - CSV');
define('MODULE_MILANDO_FILE_TITLE' , '<hr noshade="noshade"/>Dateiname');
...
// include needed functions
  ...
  class milando {
    ...
    function milando() {
      ...
      $this->enabled = ((MODULE_MILANDO_STATUS == TRUE_STRING_S) ? true : false);
    ...
    function process($file) {
        ...
        while ($products = olc_db_fetch_array($export_query)) {
            ...
            $schema .=
                        ...
                        . ';' .
                        olc_date_short($products['products_date_available']) . ';'
                        . ';'
                        . ';' .
    ...
    return array('text' =>  EXPORT_STATUS_TYPE.HTML_BR.
                            ...
                            HTML_A_START . olc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=milando') . '">' .
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = olc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_MILANDO_STATUS'");
    ...
    function install() {
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_MILANDO_FILE', 'milando.csv',  '6', '1', '', now())");
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_MILANDO_STATUS', 'true',  '6', '1', 'olc_cfg_select_option(array(\'True\', \'False\'), ', now())");
}
