<?php
/* -----------------------------------------------------------------------------------------
   $Id: kelkoo.php,v 1.1.1.1 2006/12/22 13:39:10 gswkaiser Exp $
...
define('MODULE_KELKOO_FILE_TITLE' , '<hr noshade="noshade"/>Dateiname');
...
// include needed functions
  ...
  class kelkoo {
    ...
    function kelkoo() {
      ...
      $this->enabled = ((MODULE_KELKOO_STATUS == TRUE_STRING_S) ? true : false);
    ...
    function process($file) {
        ...
        $schema ='#country=DE' . NEW_LINE.
                   '#type=basic' . NEW_LINE.
                   '#currency='.$_POST['currencies'] . NEW_LINE;
        ...
        while ($products = olc_db_fetch_array($export_query)) {
            ...
            //create content
                     $schema .=
                       ...
                       $products['products_name'] .' - '. $products['products_model']. "\t".
    ...
    return array('text' =>  EXPORT_STATUS_TYPE.HTML_BR.
                            ...
                            HTML_A_START . olc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=kelkoo') . '">' .
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = olc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_KELKOO_STATUS'");
    ...
    function install() {
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_KELKOO_FILE', 'kelkoo.txt',  '6', '1', '', now())");
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_KELKOO_STATUS', 'true',  '6', '1', 'olc_cfg_select_option(array(\'True\', \'False\'), ', now())");
}
