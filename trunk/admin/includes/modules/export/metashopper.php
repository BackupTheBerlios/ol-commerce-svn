<?php
/* -----------------------------------------------------------------------------------------
   $Id: metashopper.php,v 1.1.1.1 2006/12/22 13:39:10 gswkaiser Exp $
...
define('MODULE_METASHOPPER_TEXT_DESCRIPTION', 'Export - Metashopper (; getrennt)<br/><b>Format:</b><br/>Bestellnummer;Bezeichnung;Preis;Lieferzeit;ProduktLink;FotoLink;Beschreibung');
define('MODULE_METASHOPPER_TEXT_TITLE', 'Metashopper - CSV');
define('MODULE_METASHOPPER_FILE_TITLE' , '<hr noshade="noshade"/>Dateiname');
...
// include needed functions
  ...
  class metashopper {
    ...
    function metashopper() {
      ...
      $this->enabled = ((MODULE_METASHOPPER_STATUS == TRUE_STRING_S) ? true : false);
    ...
    function process($file) {
        ...
        $schema = 'Bestellnummer;Bezeichnung;Preis;Lieferzeit;ProduktLink;FotoLink;Beschreibung' . NEW_LINE;
    ...
    return array('text' =>  EXPORT_STATUS_TYPE.HTML_BR.
                            ...
                            HTML_A_START . olc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=metashopper') . '">' .
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = olc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_METASHOPPER_STATUS'");
    ...
    function install() {
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_METASHOPPER_FILE', 'metashopper.csv',  '6', '1', '', now())");
      olc_db_query(INSERT_INTO . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_METASHOPPER_STATUS', 'true',  '6', '1', 'olc_cfg_select_option(array(\'True\', \'False\'), ', now())");
}
