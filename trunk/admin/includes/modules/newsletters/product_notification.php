<?php
/* --------------------------------------------------------------
$Id: product_notification.php,v 1.1.1.1.2.1 2007/04/08 07:16:47 gswkaiser Exp $
...
class product_notification {
	...
	function choose_audience() {
		...
		$products_query = olc_db_query("select pd.products_id, pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where pd.language_id = '" . SESSION_LANGUAGE_ID . "' and pd.products_id = p.products_id and p.products_status = '1' order by pd.products_name");
...
';
		...
		{
			$choose_audience_string = '
	    return make_AJAX_Request_POST(FormName,"");
';	    
		}
		else
		{
...
$nID=$_GET['nID'] ;
$choose_audience_string .=
olc_draw_form('notifications',FILENAME_NEWSLETTERS,'post',
'page=' . $page . '&nID=' . $nID . '&action=confirm','onsubmit="return selectAll(\'notifications\', \'chosen[]\')"').
'	<table border="0" width="100%" cellspacing="0" cellpadding="2">
	  <tr>
	    <td align="center" class="main"><b>' . TEXT_PRODUCTS . '</b><br/>' .
olc_draw_pull_down_menu('products', $products_array, '', 'size="20" style="width: 20em;" multiple') .
'			</td>
	    <td align="center" class="main">&nbsp;<br/><a href="' . olc_href_link(FILENAME_NEWSLETTERS,
'page=' . $page . '&nID=' . $nID . '&action=confirm&global=true') . '">
				<input type="button" value="' . BUTTON_GLOBAL . '" style="width: 8em;"></a><br/><br/><br/>
				<input type="button" value="' . BUTTON_SELECT . '" style="width: 8em;" onclick="javascript:mover(\'remove\');"><br/><br/>
				<input type="button" value="' . BUTTON_UNSELECT . '" style="width: 8em;" onclick="javascript:mover(\'add\');"><br/><br/><br/>
				<input type="submit" value="' . BUTTON_SUBMIT . '" style="width: 8em;"><br/><br/>
				<a href="' . olc_href_link(FILENAME_NEWSLETTERS, 'page=' . $page . '&nID=' . $nID) . '">
				<input type="button" value="' . BUTTON_CANCEL . '" style="width: 8em;"></a>
			</td>
	    <td align="center" class="main"><b>' . TEXT_SELECTED_PRODUCTS . '</b><br/>' .
				olc_draw_pull_down_menu('chosen[]', array(), '', 'size="20" style="width: 20em;" multiple') .
'			</td>
	  </tr>
	</table>
</form>';
...
function confirm() {
	...
	$confirm_string = '<table border="0" cellspacing="0" cellpadding="2">' . NEW_LINE .
	'  <tr>' . NEW_LINE .
	'    <td class="main"><font color="#ff0000"><b>' . sprintf(TEXT_COUNT_CUSTOMERS, sizeof($audience)) . '</b></font></td>' . NEW_LINE .
	'  </tr>' . NEW_LINE .
	'  <tr>' . NEW_LINE .
	'    <td>' . olc_draw_separator('pixel_trans.gif', '1', '10') . '</td>' . NEW_LINE .
	'  </tr>' . NEW_LINE .
	'  <tr>' . NEW_LINE .
	'    <td class="main"><b>' . $this->title . '</b></td>' . NEW_LINE .
	'  </tr>' . NEW_LINE .
	'  <tr>' . NEW_LINE .
	'    <td>' . olc_draw_separator('pixel_trans.gif', '1', '10') . '</td>' . NEW_LINE .
	'  </tr>' . NEW_LINE .
	'  <tr>' . NEW_LINE .
	'    <td class="main"><tt>' . nl2br($this->content) . '</tt></td>' . NEW_LINE .
	'  </tr>' . NEW_LINE .
	'  <tr>' . NEW_LINE .
	'    <td>' . olc_draw_separator('pixel_trans.gif', '1', '10') . '</td>' . NEW_LINE .
	'  </tr>' . NEW_LINE .
	'  <tr>' . olc_draw_form('confirm', FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID'] . '&action=confirm_send') . NEW_LINE .
	'    <td align="right">';
	...
	$confirm_string .= HTML_A_START . olc_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID'] . '&action=send') . '">' . olc_image_button('button_back.gif', IMAGE_BACK) . '</a> <a href="' . olc_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . olc_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a></td>' . NEW_LINE .
	'  </tr>' . NEW_LINE .
	'</table>';
