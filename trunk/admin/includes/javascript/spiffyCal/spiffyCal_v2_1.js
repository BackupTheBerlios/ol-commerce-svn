...
var HolidaysDesc = new Array();

var MONTH_NAMES = new makeArray0('Januar','Februar','März','April','Mai','Juni','Juli','August','September',
'Oktober','November','Dezember','Jan','Feb','Mär','Apr','Mai','Jun','Jul','Aug','Sep','Okt','Nov','Dez');
...
function spiffyCalManager() {
	...
	Holidays = new Array("Dec-25","Jul-4", "Feb-14","Mar-17","Oct-31");
	HolidaysDesc = new Array("Christmas Day","Independance Day","Valentine's Day","St. Patrick's Day","Halloween");
		...
		if (formatMatchCount>1)
		{
			if (this.showHelpAlerts) {
				...
				if (date1.getTime()!=date2.getTime()) {
					var Msg='Sie haben ein mehrdeutiges Datum eingegeben.\n\n Klicken Sie "OK" für:\n'+ choice1 +'\n\noder "Cancel" für:\n'+choice2;
					if (confirm(Msg)) {
						bOK=true;
					}
					else {
		...
		if (bOK==true) {
			...
			if (dStartDate!=null) {
				...
				if (dStartDate>dThis){
					...
					if (this.showHelpAlerts) { alert(rangeMsg + 'früher als '+ strStart + '.');}
					eInput.focus();
					eInput.select();
					return false;
				}
			...
			if (dEndDate!=null) {
				...
				if (dEndDate<dThis) {
					eInput.className = "cal-TextBoxInvalid";
					if (this.showHelpAlerts) {alert(rangeMsg +'später als '+ strEnd + '.');}
					eInput.focus();
					eInput.select();
					return false;
				}
			...
			if (bRequired && bIsEmpty) {
				eInput.className = "cal-TextBoxInvalid";
				if (this.showHelpAlerts) {
					alert('Dieses Datum muss vorhanden sein.\n\nBitte geben Sie ein gültiges Datum ein.');
				}
			}
			else {
				if (!bRequired && bIsEmpty) {
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	var msNames     = new makeArray0('Januar','Februar','März','April','Mai','Juni','Juli','August','September',
	...
	*/
	...
	img_DateBtn_UP.title='Kalender anzeigen';
	img_DateBtn_OVER.title=img_DateBtn_UP.title;
	img_DateBtn_DOWN.title='Kalender verbergen';
	img_DateBtn_DISABLED.src='';
	...
	*/
	...
	function makeCalendar(intWhatMonth,intWhatYear,bViewOnly) {
		...
		if (scNN6) {
			strOutput += strTemp +'<table width="100%" border="3" class="cal-Table" cellspacing="0" cellpadding="0"><tr>';
		}
		else {
			strOutput += '<table width="100%" border="3" class="cal-Table" cellspacing="0" cellpadding="0">'+strTemp+'<tr>';
		}

		if (!(bViewOnly)) {
			strOutput += '<td class="cal-HeadCell" align="center" width="100%"><a href="javascript:'+
			this.varName+'.clearDay();"><img name="calbtn1" title="Datum löschen" src="'+
			strDefBtnImgPath+'btn_del_small.gif" border="0" width="12" height="10"></a>&nbsp;&nbsp;<a href="javascript:'+
			this.varName+'.scrollMonth(-1);" title="Einen Monat zurück">&laquo;</a>&nbsp;'+
			'<SELECT class="cal-ComboBox" NAME="cboMonth" title="Monat wählen" onChange="'+this.varName+'.changeMonth();">';
			...
			strOutput += '<\/SELECT><SELECT class="cal-ComboBox" NAME="cboYear" title="Jahr wählen" onChange="'+this.varName+'.changeYear();">';
			...
			strOutput += '<\/SELECT>&nbsp;<a href="javascript:'+
			this.varName+'.scrollMonth(1);" title="Einen Monat vor">&raquo;</a>&nbsp;&nbsp;<a href="javascript:'+
			this.varName+'.hide();"><img name="calbtn2" title="Kalender verbergen" src="'+
			strDefBtnImgPath+'btn_close_small.gif" border="0" width="12" height="10"></a><\/td><\/tr><tr><td width="100%" align="center">';
		}
		else {
		...
		strOutput += '<table width="100%" cellspacing="1" cellpadding="2" border="0"><tr>';
	...
	// writeControl -------------------------------------
	//
	function writeControl(write_in_document)
	{
		var strHold='';
		var strTemp='';
		var strTempMinDate='';
...
//debug_stop();
		...
		if (!(this.useDateRange)) {
			strTemp+=' onChange="calMgr.validateDate(document.'+this.formName+'.'+this.textBoxName+','+
			this.varName+'.required);" onBlur="calMgr.formatDate(document.'+this.formName+'.'+this.textBoxName+','+
			this.varName+'.dateFormat);" ';
		}
		else {
			...
			strTemp+=' onChange="calMgr.validateDate('+'document.'+this.formName+'.'+this.textBoxName+','+
			this.varName+'.required,'+this.varName+
			'.minDate,'+this.varName+'.maxDate);" onBlur="calMgr.formatDate(document.'+this.formName+'.'+this.textBoxName+','+
			this.varName+'.dateFormat);" ';
		}

		strHold='<input class="cal-TextBox" type="text" name="' + this.textBoxName + '"' +
		strTemp + 'size="12" value="' + this.getSelectedDate() + '">';
		if (!scIE) {
		...
		if ((this.focusClick==false) || (this.focusClick==true && this.hideButton==false))  {
			...
			strHold+='<img align="absmiddle" border="0" name="' + this.btnName + '" src="' + strBtnImg +
			'" width="'+strBtnW +'" height="'+ strBtnH +'" title="Kalender anzeigen/verbergen"></a>';
		}
		//W. Kaiser - AJAX
		if (write_in_document)
		{
	...
	{
		if (scIE) {
			var Jahr=document.forms["spiffyCal"].cboYear.options[document.forms["spiffyCal"].cboYear.selectedIndex].value;
		}
		else if (scNN) {
			var Jahr=document.spiffycalendar.document.forms["spiffyCal"].cboYear.options[document.spiffycalendar.document.forms["spiffyCal"].cboYear.selectedIndex].value;
		}
		dteCur.setFullYear(Jahr);
		domlay('spiffycalendar',1,scX,scY,this.makeCalendar(dteCur.getMonth(),dteCur.getFullYear()));
	...
	function getDayLink(blnIsGreyDate,intLinkDay,intLinkMonth,intLinkYear,bViewOnly,isWE) {
		...
		if (this.showHolidays) {isHol=calMgr.isHoliday(tempDt); if (isHol!=-1) {holTxt=' title="'+
		HolidaysDesc[isHol]+'"'; tempClass='cal-HolidayCell';}}
		if (!(this.useDateRange)) {
			if (blnIsGreyDate) {
				...
				if (isDayToday(intLinkDay)) {
					if (!(bViewOnly)) {
						templink='<td align="center" class="'+tempClass+'">' + '<a class="cal-TodayLink" '+holTxt+
						' onmouseover="self.status=\' \';return true" href="javascript:'+
						this.varName+'.changeDay(' + intLinkDay + ');">' + intLinkDay + '<\/a><\/td>';
					}
					else {
					...
					if (!(bViewOnly)) {
						templink='<td align="center" class="'+tempClass+'">' + '<a class="cal-DayLink" '+holTxt+
						' onmouseover="self.status=\' \';return true" href="javascript:'+
						this.varName+'.changeDay(' + intLinkDay + ');">' + intLinkDay + '<\/a>' +'<\/td>';
					}
					else {
...
function feiertage(Jahr)
{
	...
	var	cnt	= 0	// Zaehler fuer Feiertag

	// Von Ostern abhaengige Feiertag
	...
	// Vom 4. Advent abhaengige Feiertag
...
function setFeiertage(Jahr)
{
	...
	for (var i=0;i<Feiertage.length;i++)
	{
		with (Feiertage[i])
		{
			Holidays[i] = MONTH_NAMES[datum.getMonth()]+"-"+(datum.getDate());
			HolidaysDesc[i] = name;
		}
	}
}
