...
class olcImport
{
	function olcImport($filename,$map_filename)
	{
	...
	*/
	...
	function import()
	{
		...
		if ($fp)
		{
			if (!feof($fp))
			{
				$line_data=trim(fgets($fp,10000));		//Get first line
				$line_content=explode(CSV_SEPERATOR,$line_data);
				if ($this->map_file($line_content))
				{
					...
					while (!feof($fp))
					{
						// get line content
						$line_count++;
						$line_data=trim(fgets($fp,10000));
						if ($line_data)
						{
							$this->dataArray=explode(CSV_SEPERATOR,$line_data);
...
class olcExport extends olcImport
{
	...
	function add_header_field($field_name)
	{
		...
		$heading.=$field_name_mapped.CSV_SEPERATOR;
	}
