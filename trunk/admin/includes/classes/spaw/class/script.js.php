  ...
  // prevent from executing twice on the same editor
     ...
     if (this[editor+'_rEdit'].document.readyState == 'complete')
     {
	    //W. Kaiser - Modified for better AJAX support
	    edit_text=document.all[editor].value;
	    try
	    {
				edit_text=convert_HTML_entities(edit_text);
	    }
	    catch(e)
	    {
	    }
       ...
       this[editor+'_rEdit'].document.body.innerHTML = edit_text;
       ...
       // replace flash movies with img placeholders
       SPAW_flash2img(editor);
  ...
  function SPAW_showColorPicker(editor,curcolor) {
    return showModalDialog('<?php echo $spaw_dir?>dialogs/colorpicker.php?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value+'&request_uri='+escape(window.location.href), curcolor, 
  ...
  */
  ...
  function SPAW_hyperlink_click(editor, sender)
  {
	...
	if (a)
	{
      ...
      var naProps = showModalDialog('<?php echo $spaw_dir?>dialogs/a.php?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value+'&request_uri='+escape(window.location.href), aProps, 
  ...
  function SPAW_internal_link_click(editor, sender)
  {
	...
	if (a)
	{
      ...
      var naProps = showModalDialog('<?php echo $spaw_internal_link_script?>?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value+'&request_uri='+escape(window.location.href), aProps, 
        'dialogHeight:220px; dialogWidth:366px; resizable:yes; status:no');  
      
      if (naProps)  
      {
        if (naProps.description && !naProps.href && !naProps.name)
        {
          // insert "description" insted of the whole hyperlink
          a.outerHTML = naProps.description;
        }
        else
        {
          // edit hyperlink
	...
	{
	  // new hyperlink
    var naProps = showModalDialog('<?php echo $spaw_internal_link_script?>?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value+'&request_uri='+escape(window.location.href), null, 
      'dialogHeight:220px; dialogWidth:366px; resizable:no; status:no');  
    ...
    if (naProps)  
    {
		  ...
		  {
        var selection = window.frames[editor+'_rEdit'].document.selection.createRange();
        selection.pasteHTML(naProps.description);      
		  }
		  else
		  {
      ...
      if (imgSrc.toLowerCase().indexOf('.swf') == -1)
      {
        ...
        var flash = document.createElement('EMBED');
    	  ...
    	  */
    	  ...
    	  flash.setAttribute('src', 'lib/util/spacer100.gif?imgtype=flash&src='+imgSrc);
    	  flash.style.cssText = "border: 1px solid #000000; background: url(lib/util/flash.gif);";
    	  var selection = window.frames[editor+'_rEdit'].document.selection.createRange();
    	  selection.pasteHTML(flash.outerHTML);
    	}
    }
  ...
  function SPAW_image_prop_click(editor, sender)
  {
    ...
    {
      var iProps = {};
      if (im.tagName == 'IMG' && im.src.indexOf("spacer100.gif?imgtype=flash")==-1)
      {
        // regular image
      ...
      {
        // flash
        iProps.src = im.src.substring(im.src.indexOf("src=")+4);
        iProps.type = 'flash';
        if (im.attributes.getNamedItem("width").specified)
          iProps.width = (im.style.width)?im.style.width:im.width;
        if (im.attributes.getNamedItem("height").specified)
          iProps.height = (im.style.height)?im.style.height:im.height;
      }
      ...
      if (niProps)  
      {
        ...
        if (niProps.height)
          ...
          im.removeAttribute("widht",0);

        im.style.height = (niProps.height)?niProps.height:'';

        if (im.tagName == 'IMG' && im.src.substring(im.src.length-4).toLowerCase() != ".swf")
        {
        ...
        {
          // flash
          im.src = "lib/util/spacer100.gif?imgtype=flash&src=" + ((niProps.src)?niProps.src:'');
        }
      }      
  ...
  function SPAW_image_popup_click(editor, sender)
  {
    ...
    if(imgSrc != null)    
	{
		if (a)
		{
			// edit hyperlink
			a.href="#";
			a.onclick="javascript:window.open('<?php echo $spaw_img_popup_url?>?img_url="+SPAW_stripAbsoluteUrlFromImg(editor, imgSrc)+"','Image','width=500,height=300,scrollbars=no,toolbar=no,location=no,status=no,resizable=yes,screenX=120,screenY=100');return false;";
		}
		else
		{
			var a;
  ...
  function SPAW_codesnippet_change(editor, sender)
  {
    if (sender.selectedIndex != 0)
    {
      var codesnippet = sender.options[sender.selectedIndex].value;
      ...
      if (window.frames[editor+'_rEdit'].document.selection.type == "Control")
      {
        codesnippet = codesnippet.replace('%%CURRENT_CONTENT%%', selection(0).outerHTML);
  			selection(0).outerHTML = codesnippet;
      }
      else
      {
        codesnippet = codesnippet.replace('%%CURRENT_CONTENT%%', selection.htmlText);
        selection.pasteHTML(codesnippet);
      }
  ...
  function SPAW_getHtmlValue(editor, thefield)
  {
    ...
    if(document.all['SPAW_'+editor+'_editor_mode'].value == 'design')
    {
      // wysiwyg
  	  ...
  	  SPAW_img2flash(editor);
	    <?php if ($spaw_disable_xhtml) { ?>
      htmlvalue = this[editor+'_rEdit'].document.body.innerHTML;
      <?php } else { ?>
      var bodyclone = this[editor+'_rEdit'].document.body;//.cloneNode(2);
      htmlvalue = SPAW_dom2xml(editor, bodyclone);
      <?php } ?>
    }
    else
    {
  ...
  function SPAW_confirm(editor,block,message) {
    return showModalDialog('<?php echo $spaw_dir?>dialogs/confirm.php?lang=' + document.all['SPAW_'+editor+'_lang'].value + '&theme=' + document.all['SPAW_'+editor+'_theme'].value + '&block=' + block + '&message=' + message +'&request_uri='+escape(window.location.href), null, 'dialogHeight:100px; dialogWidth:300px; resizable:no; status:no');  
  }
  ...
  // returns well-formed xml (xhtml) from dom object
  function SPAW_dom2xml(editor, domobject)
  {
    var xbuf = '';
    for (var i=0; i<domobject.childNodes.length; i++)
    {
      var chnode = domobject.childNodes[i];
      if (chnode.nodeType == 3) // text node
      {
        xbuf += chnode.nodeValue.replace('&','&amp;').replace(/\u00A0/g, '&nbsp;').replace('<','&lt;').replace('>','&gt;');
      }
      ...
      else if (chnode.nodeType == 1) // html element
      {
        if (chnode.getAttribute("__spawprocessed") == null) // workaround to prevent elements from doubling
        {
          ...
          for(var j=0; j<chnode.attributes.length; j++)
          {
            ...
            if ((attr.nodeValue != null || chnode.getAttribute(attr.nodeName, 2)!=null) && attr.specified && attr.nodeName.toLowerCase()!="__spawprocessed" && attr.nodeName.toLowerCase().indexOf("_moz"))
            {
              var attrval = (attr.nodeValue != null)?attr.nodeValue:chnode.getAttribute(attr.nodeName, 2);
              if ((chnode.nodeName.toLowerCase() == "img" || chnode.nodeName.toLowerCase() == "embed") && attr.nodeName.toLowerCase() == "src")
                attrval = SPAW_stripAbsoluteUrlFromImg(editor, attrval);
              if (chnode.nodeName.toLowerCase() == "a" && attr.nodeName.toLowerCase() == "href")
                attrval = SPAW_stripAbsoluteUrl(editor, attrval);
          ...
          if (chnode.tagName.toLowerCase() != "script")
          {
            if (chnode.childNodes.length>0)
              xbuf += "<" + SPAW_trim(chnode.tagName.toLowerCase() + attr_str) + ">" + SPAW_dom2xml(editor, chnode) + "</" + chnode.tagName.toLowerCase() + ">";
            else if (chnode.tagName.indexOf("/") == -1) // empty tag (sometimes ending tag is passed as a separate node
            {
              ...
              {
                xbuf += "<" + SPAW_trim(chnode.tagName.toLowerCase() + attr_str) + " />";
              }
              else
              {
                xbuf += "<" + SPAW_trim(chnode.tagName.toLowerCase() + attr_str) + "></" + chnode.tagName.toLowerCase() + ">";
              }
            }
          }
          else // script
          {
            xbuf += "<" + SPAW_trim(chnode.tagName.toLowerCase() + attr_str) + ">" + chnode.innerHTML + "</" + chnode.tagName.toLowerCase() + ">";                    
          }
        }
      }
/*      else
...
*/
  ...
  function SPAW_img2flash(editor)
  {
    var imgs_elm = this[editor+'_rEdit'].document.body.getElementsByTagName("IMG");
    ...
    for (var i=0; i<imgs.length; i++)
    {
      ...
      {
        var flash = this[editor+'_rEdit'].document.createElement('EMBED');
    	  flash.setAttribute('type','application/x-shockwave-flash'); // ie adds this automatically
    	  flash.setAttribute('src',imgs[i].src.substring(imgs[i].src.indexOf("src=")+4));
    	  flash.setAttribute('play','true');
    	  flash.setAttribute('loop','true');
    	  flash.setAttribute('menu','true');
    	  if (imgs[i].attributes.getNamedItem("width").specified)
      	  flash.setAttribute('width',imgs[i].width);
    	  if (imgs[i].attributes.getNamedItem("height").specified)
  ...
  function SPAW_flash2img(editor)
  {
    var flashs_elm = this[editor+'_rEdit'].document.body.getElementsByTagName("EMBED");
    ...
    for (var i=0; i<flashs.length; i++)
    {
      ...
      {
    	  var flash = this[editor+'_rEdit'].document.createElement("IMG");
    	  flash.setAttribute('src', 'lib/util/spacer100.gif?imgtype=flash&src='+flashs[i].src);
    	  flash.style.cssText = "border: 1px solid #000000; background: url(lib/util/flash.gif);";
    	  if (flashs[i].attributes.getNamedItem("width").specified)
      	  flash.setAttribute('width',flashs[i].width);
    	  if (flashs[i].attributes.getNamedItem("height").specified)
      	  flash.setAttribute('height',flashs[i].height);

        flashs[i].parentNode.replaceChild(flash, flashs[i]); 
    	  //flashs[i].outerHTML = flash.outerHTML;
