<?php
/* --------------------------------------------------------------
   $Id: payment_module_info.php,v 1.1.1.1.2.1 2007/04/08 07:16:41 gswkaiser Exp $   
  ...
  class paymentModuleInfo {
    ...
    function paymentModuleInfo($pmInfo_array) {
      ...
      for ($i = 0, $n = sizeof($pmInfo_array) - 1; $i < $n; $i++) {
        $key_value_query = olc_db_query("select configuration_title, configuration_value, configuration_description from " . TABLE_CONFIGURATION . " where configuration_key = '" . $pmInfo_array[$i] . APOS);
