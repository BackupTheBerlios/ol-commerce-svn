<?php
/* --------------------------------------------------------------
   $Id: phplot.php,v 1.1.1.1.2.1 2007/04/08 07:16:41 gswkaiser Exp $   
...
*************************************** */
	...
	function DrawText($which_font,$which_angle,$which_xpos,$which_ypos,$which_color,$which_size,$which_text,$which_halign='left',$which_valign='') {
		...
		if ($this->use_ttf == 1 ) { 
			...
			$str = split(NEW_LINE,$which_text); //multiple lines submitted by Remi Ricard
	...
	function SetMargins() {
		...
		$str = split(NEW_LINE,$this->title_txt); 
		$nbLines = count($str); 
	...
	function FormatYTickLabel($which_ylab) { 
		switch ($this->y_grid_label_type) {
			...
			case "right":
				...
				//$ylab = str_pad($which_ylab,$this->y_label_width,BLANK,STR_PAD_LEFT); //PHP4 only
