...
class campaigns {
	...
	function getCampaigns() {
		...
		$campaign_query = olc_db_query($campaign_query);
		while ($campaign_data = olc_db_fetch_array($campaign_query)) {
	...
	function getTotalLeads() {
		...
		$selection = " and ci.customers_info_date_account_created>'".olc_db_input(date("Y-m-d", $this->startDate))."'"." and ci.customers_info_date_account_created<'".olc_db_input(date("Y-m-d", $end))."'";
		...
		$lead_query = olc_db_query($lead_query);
		$lead_data = olc_db_fetch_array($lead_query);
	...
	function getTotalSells() {
		$end = mktime(0, 0, 0, date("m", $this->endDate), date("d", $this->endDate) + 1, date("Y", $this->endDate));
		$selection = " and o.date_purchased>'".olc_db_input(date("Y-m-d", $this->startDate))."'"." and o.date_purchased<'".olc_db_input(date("Y-m-d", $end))."'";
		$status = "";
		if ($this->status > 0)
		...
		$sale_query = olc_db_query($sale_query);
		$sale_data = olc_db_fetch_array($sale_query);
	...
	function getSells($date_start, $date_end = '', $type) {
		...
		switch ($type) {
			...
			case 2 :
			case 3 :
				$selection = " and o.date_purchased>'".olc_db_input(date("Y-m-d", $date_start))."'"." and o.date_purchased<'".olc_db_input(date("Y-m-d", $date_end))."'";
			...
			case 4 :
				...
				$selection = " and o.date_purchased>'".olc_db_input(date("Y-m-d", $date_start))."'"." and o.date_purchased<'".olc_db_input(date("Y-m-d", $end))."'";
				break;
		...
		$sell_query = olc_db_query($sell_query);
		$sell_data = olc_db_fetch_array($sell_query);
		...
		$late_sell_query = olc_db_query($late_sell_query);
		$late_sell_data = olc_db_fetch_array($late_sell_query);
	...
	function getLeads($date_start, $date_end = '', $type) {
		...
		switch ($type) {
			...
			case 2 :
			case 3 :
				$selection = " and ci.customers_info_date_account_created>'".olc_db_input(date("Y-m-d", $date_start))."'"." and ci.customers_info_date_account_created<'".olc_db_input(date("Y-m-d", $date_end))."'";

				break;

			case 4 :
				$end = mktime(0, 0, 0, date("m", $date_start), date("d", $date_start) + 1, date("Y", $date_start));
				$selection = " and ci.customers_info_date_account_created>'".olc_db_input(date("Y-m-d", $date_start))."'"." and ci.customers_info_date_account_created<'".olc_db_input(date("Y-m-d", $end))."'";
