<?php
/* --------------------------------------------------------------
   $Id: carpinc.php,v 1.1.1.1.2.1 2007/04/08 07:16:39 gswkaiser Exp $
...
class RSSParser {
	...
	function CheckFilter($lookfor,$field) {
		if (strlen($field)) {
			...
			if (strpos(strtolower($this->title.BLANK.$this->description),$lookfor)!==false) return 1;
		}
		return 0;
	}
			...
			if ($this->itemcount<$carpconf['maxitems']) {
				...
				if (!$filterblock) {
					...
					$thisitem.="<a href=\"".trim($this->link)."\"".$this->linktargets[$carpconf['linktarget']].
						...
						'>'.$this->title.HTML_A_END.(strlen($carpconf['linkdiv'])?"</div>\n":($carpconf['showdesc']?"<br/>\n":''));
...
function OpenCacheWrite() {
	...
	} else CarpError("Can't open cache file: ".$cachefile);
	return $j;
}
	...
	if ($fp=OpenRSSFeed($url)) {
		...
		while ($data=preg_replace("/&(?!lt|gt|amp|apos|quot)(.*\b)/is","&amp;\\1\\2",fread($fp,4096))) {
			if (!xml_parse($xml_parser,$data,feof($fp))) {
				CaRPError("XML error: ".xml_error_string(xml_get_error_code($xml_parser))." at line ".
		...
		if ($showit) CarpOutput($parser_output);
		if ($cache) {
