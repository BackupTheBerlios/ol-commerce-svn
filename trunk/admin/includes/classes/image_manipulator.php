<?php
/* ----------------------------------------------------------------------------------------
$Id: image_manipulator.php,v 1.1.1.1.2.1 2007/04/08 07:16:41 gswkaiser Exp $

OL-Commerce http://www.ol-commerce.de, http://www.seifenparadies.de
...
-----------------------------------------------------------------------------------------
modification for "improved picture quality" date: 2005/08/17

changed all 'imagecreate' calls to 'imagecreatetruecolor' and
'imagecopyresized' to 'imagecopyresampled'

to use this version you need PHP 4.0.6 or better and GD-Library 2.0.1 or better!

modified by Siegfried Werginz <siegfried@werginz.de>
---------------------------------------------------------------------------------------*/

class image_manipulation
{
	function image_manipulation($resource_file,$max_width,$max_height,$destination_file=EMPTY_STRING,
	$compression=80,$transform=EMPTY_STRING)
	{
	...
	function bevel($edge_width=10,$light_colour="FFFFFF",$dark_colour="000000")
	{
		...
		$this->dark=imagecreatetruecolor($this->q,$this->r);
		$this->nadir=imagecolorallocate($this->dark,$this->dr,$this->dg,$this->db);
		$this->light=imagecreatetruecolor($this->q,$this->r);
		...
		for($this->pixel=0;$this->pixel<$this->edge;$this->pixel++)
		{
			...
			ImageCopyMerge($this->t,$this->dark,$this->q-($this->pixel+1),$this->pixel,0,0,1,$this->r-(2*$this->pixel),
			max(0,$this->opac-10));
			ImageCopyMerge($this->t,$this->dark,$this->pixel,$this->r-($this->pixel+1),0,0,$this->q-(2*$this->pixel),1,
			max(0,$this->opac-10));
		}
	...
	function ellipse($bg_colour="FFFFFF")
	{
		...
		$this->dot=imagecreatetruecolor(6,6);
	...
	function round_edges($edge_rad=3,$bg_colour="FFFFFF",$anti_alias=1)
	{
		...
		$this->dot=imagecreatetruecolor(1,1);
		$this->dot_base=ImageColorAllocate($this->dot,$this->br,$this->bg,$this->bb);
		$this->zenitha=ImageColorClosest($this->t,$this->br,$this->bg,$this->bb);
	...
	function merge($merge_img=EMPTY_STRING,$x_left=0,$y_top=0,$merge_opacity=70,$trans_colour="FF0000",
	$target_w=0,$target_h=0)
	{
		...
		{
			case 1:
				$this->mm=imagecreatefromgif($this->mi);
				break;
			case 2:
				$this->mm=imagecreatefromjpeg($this->mi);
				break;
			case 3:
				$this->mm=imagecreatefrompng($this->mi);
				break;
		}
		if ($trans_colour)
		{
			for ($this->ypo=0;$this->ypo<$this->mh;$this->ypo++)
			{
				for ($this->xpo=0;$this->xpo<$this->mw;$this->xpo++)
				{
					...
					if (($this->indx_rgb['red']==$this->tr)&&($this->indx_rgb['green']==$this->tg)&&
						($this->indx_rgb['blue']==$this->tb))
					{
		...
		else
		{
			...
			{
				//Resize merge image if size does not match target sizes
				if ($target_w==0 )
				{
					$target_w=$this->mw;
				}
				...
				elseif (!$modify_image)
				{
					$modify_image=$target_h<>$this->mh;
				}
				if ($modify_image)
				{
					$fact_w=($this->mw/$target_w);
					$fact_h=($this->mh/$target_h);
					$bigger_width=$fact_w>$fact_h;
					$width=($bigger_width)?$target_w:round($this->mw/$fact_h);//width
					$height=($bigger_width)?round($this->mh/$fact_w):$target_h;//height
					$temp=imagecreatetruecolor($width,$height);
					imagecopyresampled($temp,$this->mm,0,0,0,0,$width,$height,$this->mw,$this->mh);
					imagedestroy($this->mm);
					$this->mm=imagecreatetruecolor($width,$height);
					imagecopy ($this->mm,$temp,0,0,0,0,$width,$height);
					imagedestroy($temp);
				}
			}
			else 
	...
	function frame($light_colour="FFFFFF",$dark_colour="000000",$mid_width=4,$frame_colour=EMPTY_STRING)
	{
		$this->rw=$mid_width;
		$this->dh=$dark_colour;
		...
		$this->mid=($this->frc==EMPTY_STRING)?ImageColorClosest($this->t,($this->gr+$this->fr)/2,
		($this->gg+$this->fg)/2,($this->gb+$this->fb)/2):ImageColorClosest($this->t,$this->hex2rgb(substr($this->frc,0,2)),
		$this->hex2rgb(substr($this->frc,2,2)),$this->hex2rgb(substr($this->frc,4,2)));
	...
	function motion_blur($num_blur_lines,$background_colour="FFFFFF")
	{
		...
		for($this->lst=0;$this->lst<$this->nbl;$this->lst++)
		{
			...
			{
				imagecopymerge($this->w,$this->t,$this->rsw+(2*$this->lst)+1,$this->yst+(2*$this->lst)+2,$this->q-1,
				$this->yst*$this->rati,1,1,$this->opacit);
			}
			for($this->xst=0;$this->xst<$this->rsw;$this->xst++)
			{
				imagecopymerge($this->w,$this->t,$this->xst+(2*$this->lst)+1,$this->rsh+(2*$this->lst)+1,$this->xst*$this->rati,
				$this->r-1,1,1,$this->opacit);
			}
		}
	...
	function manipulate()
	{
		if($this->c!==EMPTY_STRING&&$this->s!==Null)
		{
