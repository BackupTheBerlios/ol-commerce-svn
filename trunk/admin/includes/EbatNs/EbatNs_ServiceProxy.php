<?php
// $Id: EbatNs_ServiceProxy.php,v 1.1.1.1.2.1 2007/04/08 07:16:39 gswkaiser Exp $
/* $Log: EbatNs_ServiceProxy.php,v $
 ...
 */
// 
// auto-generated 17.11.2006 13:30 
// Ebay-Schema Version 487
//
/**
	...
	function getErrorsToString($response, $asHtml = false, $addSlashes = true)
	{
		...
		if (count($response->Errors))
			foreach ($response->Errors as $errorEle)
				$errmsg .= '#' . $errorEle->getErrorCode() . ' : ' . ($asHtml ? htmlentities($errorEle->getLongMessage()) :  $errorEle->getLongMessage()) . ($asHtml?HTML_BR : "\r\n");
 ...
 * @param GetCategory2FinanceOfferRequestType $request 
 */
	function &GetCategory2FinanceOffer($request)
	{
			$request->setVersion(EBAY_WSDL_VERSION);
		return ($res = & $this->call('GetCategory2FinanceOffer', $request));
 ...
 * @param GetFinanceOffersRequestType $request 
 */
	function &GetFinanceOffers($request)
	{
			$request->setVersion(EBAY_WSDL_VERSION);
		return ($res = & $this->call('GetFinanceOffers', $request));
 ...
 * @param GetMyeBayRequestType $request 
 */
	function &GetMyeBay($request)
	{
			$request->setVersion(EBAY_WSDL_VERSION);
		return ($res = & $this->call('GetMyeBay', $request));
 ...
 * @param GetRecommendationsRequestType $request 
 */
	function &GetRecommendations($request)
	{
			$request->setVersion(EBAY_WSDL_VERSION);
		return ($res = & $this->call('GetRecommendations', $request));
 ...
 * @param SetPromotionRulesRequestType $request 
 */
	function &SetPromotionRules($request)
	{
			$request->setVersion(EBAY_WSDL_VERSION);
		return ($res = & $this->call('SetPromotionRules', $request));
