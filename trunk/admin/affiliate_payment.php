<?php
/*------------------------------------------------------------------------------
$Id: affiliate_payment.php,v 1.1.1.1.2.1 2007/04/08 07:16:24 gswkaiser Exp $
...
?>
      ...
      <tr>
<?php echo olc_rm('status', FILENAME_AFFILIATE_PAYMENT, olc_get_all_get_params(array('action')) . 'action=update_payment'); ?>
...
if (olc_db_num_rows($affiliate_history_query)) {
	while ($affiliate_history = olc_db_fetch_array($affiliate_history_query)) {
		...
		'            <td class="smallText">' . (olc_not_null($affiliate_history['affiliate_old_value']) ? $payments_status_array[$affiliate_history['affiliate_old_value']] : HTML_NBSP) . '</td>' . NEW_LINE .
