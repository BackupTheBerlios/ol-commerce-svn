<?php
/* --------------------------------------------------------------
$Id: auctions_list_sold_not.php,v 1.1.1.1.2.1 2007/04/08 07:16:25 gswkaiser Exp $
...
$auctionssql = SELECT_ALL.TABLE_AUCTION_LIST."
WHERE auction_id NOT IN (SELECT auction_id FROM '.TABLE_AUCTION_DETAILS.') AND ended =1 ".$ordersql;
...
while ($auctions_values = olc_db_fetch_array($myauctions))
{
	...
	$values_text => array (
	array (
		$value_text => $auctions_values[$quantity_text].$x_text.$auctions_values[$auction_title_text],
		$link_text => EBAY_SERVER.EBAY_VIEWITEM.$auctions_values[$auction_id_text].APOS,
		$linkattribute_text => $target_blank_text,
		$attributes_text => $align_right_text),
	array (
		$value_text => $auctions_values[$auction_id_text],
		$link_text => $getItem_text.$auctions_values[$auction_id_text],
		$linkattribute_text => $target_blank_text,
		$attributes_text => $align_right_text),
	array (
...
$page_header_subtitle=AUCTIONS_TEXT_SUB_HEADER_AUCTION_SOLD_NOT;
