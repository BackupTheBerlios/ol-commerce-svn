...
{
	$Table = olc_db_query("show tables like '" . $table_name . "'");
	if(olc_db_fetch_row($Table) === false)
	{
		return(false);
	} else {
		return(true);
...
{
	$Table = olc_db_query("show columns from $table LIKE '" . $column . "'");
	if(olc_db_fetch_row($Table) === false)
	{
		return(false);
	} else {
...
function SendCategories ()
{
	...
	while ($cat = olc_db_fetch_array($cat_query))
	{
		...
		$detail_query = olc_db_query(
		"select categories_id, language_id,
    categories_name,
    ...
    categories_meta_keywords, " .
		...
		TABLE_LANGUAGES .
		" where " .
		TABLE_CATEGORIES_DESCRIPTION . ".categories_id=" . $cat['categories_id'] . "
		and " . TABLE_LANGUAGES . ".languages_id=" . TABLE_CATEGORIES_DESCRIPTION . ".language_id");

		while ($details = olc_db_fetch_array($detail_query))
		{
			$schema .= "<CATEGORIES_DESCRIPTION ID='" . $details["language_id"] ."' CODE='" . $details["lang_code"] .
			"' NAME='" . $details["lang_name"] . "'>\n";
...
function SendProducts ()
{
	...
	while ($products = olc_db_fetch_array($orders_query))
	{
		...
		'<PRODUCT_DEEPLINK>'.	$deep_link.$products_id.
		'</PRODUCT_DEEPLINK>' . "\n" .
		...
		if ($products_image)
		{
			...
			*/
			$schema .=
			'<PRODUCT_IMAGE_ORIGINAL>'.olc_image($http_catalog.DIR_WS_ORIGINAL_IMAGES.$products_image).
			'</PRODUCT_IMAGE_ORIGINAL>'. "\n".
			'<PRODUCT_IMAGE_SMALL>'.olc_image($http_catalog.DIR_WS_INFO_IMAGES.$products_image).
			'</PRODUCT_IMAGE_SMALL>'. "\n" .
			'<PRODUCT_IMAGE_POPUP>'.olc_image($http_catalog.DIR_WS_POPUP_IMAGES.$products_image).
			'</PRODUCT_IMAGE_POPUP>'. "\n" .
			'<PRODUCT_IMAGE_THUMBNAIL>'.olc_image($http_catalog.DIR_WS_THUMBNAIL_IMAGES.$products_image).
			'</PRODUCT_IMAGE_THUMBNAIL>'. "\n";
			//W. Kaiser - pictures-on-the-fly
		}
		...
		for ($i=1,$n=sizeof($customers_status);$i<$n; $i++)
		{
			...
			{
				$schema .=
				"<PRODUCT_GROUP_PRICES ID='".$customers_status[$i]['id']."' NAME='".$customers_status[$i]['text']. "'>". "\n";
				$group_price_query=olc_db_query("SELECT * FROM " .
				TABLE_PERSONAL_OFFERS_BY_CUSTOMERS_STATUS . $customers_status[$i]['id'].
...
function ShowHTMLMenu ()
{
	...
	$target='"'.$target.'>';
	$parameter.='&action=';
	$page_header_title='CAO-Faktura - OL-Commerce Shopanbindung';
	$page_header_subtitle='Version '.$version_nr.' Stand : '.$version_datum;
	$href='<a href="';
	$br='<br>';
	$href_end='</a>'.$br;
	$href_end_br=$href_end_br;
	$exporte='XML-Exporte ';
	$target_exporte=$target.$exporte;
	$main_content=
	$br.'<b>Mögliche Funktionen :</b>'.$br.$br.
	$href.olc_href_link($Url,$parameter.'version').$target_exporte.'Scriptversion'.$href_end_br.
	$href.olc_href_link($Url,$parameter.'manufacturers_export').$target_exporte.'Hersteller'.$href_end.
	$href.olc_href_link($Url,$parameter.'categories_export').$target_exporte.'Kategorien'.$href_end.
	$href.olc_href_link($Url,$parameter.'products_export').$target_exporte.'Produkte'.$href_end.
	$href.olc_href_link($Url,$parameter.'customers_export').$target_exporte.'Kunden'.$href_end.
	$href.olc_href_link($Url,$parameter.'customers_newsletter_export').$target_exporte.'Kunden-Newsletter'.$href_end_br.
	$href.olc_href_link($Url,$parameter.'orders_export').$target_exporte.'Bestellungen'.$href_end_br.
	$href.olc_href_link($Url,$parameter.'config_export').$target_exporte.'Shop-Konfiguration'.$href_end_br.
	$href.olc_href_link($Url,$parameter.'update_tables').$target.'MySQL-Tabellen aktualisieren'.$href_end.'
';
	if (INTEGRATE_INTO_OLC)
	{
		chdir('admin');
...
function UpdateTables ()
{
	...
	$sql[13] = 'CREATE TABLE ' . TABLE_PREFIX_INDIVIDUAL.
	'cao_log ( id int(11) NOT NULL auto_increment, date datetime NOT NULL default "0000-00-00 00:00:00",'.
...
function olc_try_upload ($file = '', $destination = '',$permissions = '777', $extensions = '')
{
...
function remove_product($product_id)
{
	...
	//W. Kaiser - pictures-on-the-fly
	$image_dirs=array(DIR_FS_CATALOG_ORIGINAL_IMAGES);
	if (defined('BIG_IMAGE_SUBDIR'))
	{
		$image_subdir = BIG_IMAGE_SUBDIR;
		if (substr($image_subdir, -1) != '/') $image_subdir .= '/';
		$image_dirs=array_merge($image_dirs,array(DIR_WS_PRODUCT_IMAGES.$image_subdir));
	}
	$imagecache_text='imagecache';
	if (PRODUCT_IMAGE_ON_THE_FLY=='true')
	{
		$merge_array=array(DIR_WS_PRODUCT_IMAGES.$imagecache_text.'/');
	}
	else
	{
		$merge_array=array(DIR_WS_INFO_IMAGES,DIR_WS_POPUP_IMAGES.DIR_WS_THUMBNAIL_IMAGES);
	}
	$image_dirs=array_merge($image_dirs,$merge_array);

	$product_image_query = olc_db_query("select products_image from " . TABLE_PRODUCTS .
	" where products_id = '" . olc_db_input($product_id) . "'");
	while ($product_image = olc_db_fetch_array($product_image_query))
	{
		$products_image=$product_image['products_image'];
		for ($i=0,$n=sizeof($image_dirs);$i<$n;$i++)
		{
			$dir=$image_dirs[$i];
			$is_cache=strpos($dir,$imagecache_text)!==false;
			if ($is_cache)
			{
				$handle=strrpos($products_image,'.');
				if ($handle!==false)
				{
					$products_image=substr($products_image,0,$handle);
				}
				$products_image=basename($products_image).'_';
				$handle = opendir($dir);
				while ($readdir = readdir($handle))
				{
					if($readdir != '.' && $readdir != '..')
					{
						if (strpos($readdir,$products_image)!==false)
						{
							$path = $dir.'/'.$readdir;
							if (is_file($path))
							{
			...
			{
				$products_image=$dir.$products_image;
				if (file_exists($products_image))
				{
					@unlink($products_image);
				}
			}
		}
	}
	$delete_from="delete from ";
	$where_products_id=" where products_id = '" . olc_db_input($product_id) . "'";
	olc_db_query($delete_from . TABLE_SPECIALS . $where_products_id);
	olc_db_query($delete_from . TABLE_PRODUCTS . $where_products_id);
	olc_db_query($delete_from . TABLE_PRODUCTS_TO_CATEGORIES . $where_products_id);
	olc_db_query($delete_from . TABLE_PRODUCTS_DESCRIPTION . $where_products_id);
	olc_db_query($delete_from . TABLE_PRODUCTS_ATTRIBUTES . $where_products_id);
	olc_db_query($delete_from . TABLE_CUSTOMERS_BASKET . $where_products_id);
	olc_db_query($delete_from . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . $where_products_id);
	//W. Kaiser - pictures-on-the-fly
	// get statuses
	$customers_statuses_array = array(array());

	$customers_statuses_query = olc_db_query("select * from " . TABLE_CUSTOMERS_STATUS .
	" where language_id = '".$LangID."' order by customers_status_id");
	...
	{
		olc_db_query("delete from personal_offers_by_customers_status_" . $i .
		" where products_id = '" . olc_db_input($product_id) . "'");
	}
	$product_reviews_query = olc_db_query("select reviews_id from " . TABLE_REVIEWS .
	" where products_id = '" . olc_db_input($product_id) . "'");
...
function ManufacturersUpdate ()
{
	...
	if (isset($manufacturers_id))
	{
		...
		$languages_query = olc_db_query("select languages_id, name, code, image, directory from " . TABLE_LANGUAGES .
		" order by sort_order");
		...
		for ($i = 0, $n = sizeof($languages); $i < $n; $i++)
		{
			...
			TABLE_MANUFACTURERS_INFO . " where manufacturers_id='" . $manufacturers_id .
			...
			if (isset($_POST['manufacturers_url'][$language_id]))
			$manufacturers_url=olc_db_prepare_input($_POST['manufacturers_url'][$language_id]);
			if (isset($_POST['url_clicked'][$language_id]))
			$url_clicked=olc_db_prepare_input($_POST['url_clicked'][$language_id]);
			if (isset($_POST['date_last_click'][$language_id]))
			$date_last_click=olc_db_prepare_input($_POST['date_last_click'][$language_id]);
			...
			elseif ($exists==1) // Update
			{
				olc_db_perform(TABLE_MANUFACTURERS_INFO, $sql_data_array, 'update', 'manufacturers_id = \'' .
				olc_db_input($manufacturers_id) . '\' and languages_id = \'' . $language_id . '\'');
			}
		}
...
function ManufacturersErase ()
{
	...
	if (isset($ManID))
	{
		// Hersteller loeschen
		$delete_from="delete from ";
		$where_manufacturers_id=" where manufacturers_id = '" . (int)$ManID . "'";
		olc_db_query($delete_from . TABLE_MANUFACTURERS . $where_manufacturers_id);
		olc_db_query($delete_from . TABLE_MANUFACTURERS_INFO . $where_manufacturers_id);
		// Herstellerverweis in den Artikeln loeschen
		olc_db_query("update " . TABLE_PRODUCTS . " set manufacturers_id = ''".$where_manufacturers_id);
	...
	if (isset($ProdID))
	{
		...
		*/
		...
		if ($sp = olc_db_fetch_array($sp_query))
		{
			...
			print_xml_status (0, $_POST['action'], 'OK', 'SQL_UPDATE', '', '');
		}
		else
		{
			// Neuanlage
...
function ProductsSpecialPriceErase ()
{
	...
	if (isset($ProdID))
	{
		olc_db_query("delete from " . TABLE_SPECIALS . " where products_id = '" . (int)$ProdID . "'");
		print_xml_status (0, $_POST['action'], 'OK', '', '', '');
	}
	else
	{
...
function CategoriesErase ()
{
	...
	{
		// Categorie loeschen
		$res1 = olc_db_query("delete from " . TABLE_CATEGORIES . " where categories_id='" . $CatID . "'");
		// ProductsToCategieries loeschen bei denen die Categorie = ... ist
		$res2 = olc_db_query("delete from " . TABLE_PRODUCTS_TO_CATEGORIES . " where categories_id='" . $CatID . "'");
		// CategieriesDescription loeschenm bei denen die Categorie = ... ist
		$res3 = olc_db_query("delete from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id='" . $CatID . "'");
...
function Prod2CatErase ()
{
	...
	if (isset($ProdID) && isset($CatID))
	{
		$res = olc_db_query("delete from " . TABLE_PRODUCTS_TO_CATEGORIES . " where products_id='" . $ProdID ."' and categories_id='" . $CatID . "'");
		print_xml_status (0, $_POST['action'], 'OK', '', 'SQL_RES', $res);
	}
	else
	{
...
if (LOGGER)
{
	...
	olc_db_query("INSERT INTO ' . TABLE_PREFIX_INDIVIDUAL. 'cao_log
	              (date,user,pw,method,action,post_data,get_data) VALUES
	              (NOW(),'".$user."','".$password."','".
	$_SERVER['REQUEST_METHOD']."','".$_POST['action']."','".$pdata."','".$gdata."')");
}
