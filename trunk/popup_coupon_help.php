...
/* -----------------------------------------------------------------------------------------
   $Id: popup_coupon_help.php,v 1.1.1.1.2.1 2007/04/08 07:16:18 gswkaiser Exp $
...
$coupon_query = olc_db_query("select * from ".TABLE_COUPONS." where coupon_id = '".(int)$_GET['cID'].APOS);
$coupon = olc_db_fetch_array($coupon_query);
$coupon_desc_query = olc_db_query("select * from ".TABLE_COUPONS_DESCRIPTION." where coupon_id = '".(int)$_GET['cID']."' and language_id = '".SESSION_LANGUAGE_ID.APOS);
...
if (olc_not_null($coupon_desc['coupon_description']))
...
$coupon_get = olc_db_query("select restrict_to_products from ".TABLE_COUPONS."  where coupon_id='".(int)$_GET['cID'].APOS);
$get_result = olc_db_fetch_array($coupon_get);

$pr_ids = split("[,]", $get_result['restrict_to_products']);
for ($i = 0; $i < count($pr_ids); $i ++) {
	$result = olc_db_query("SELECT * FROM ".TABLE_PRODUCTS." p, ".TABLE_PRODUCTS_DESCRIPTION." pd WHERE p.products_id = pd.products_id and pd.language_id = '".SESSION_LANGUAGE_ID."'and p.products_id = '".$pr_ids[$i].APOS);
	if ($row = olc_db_fetch_array($result)) {
		$prods .= HTML_BR.$row["products_name"];
	}
}
if ($prods == '')
	$prods = '<br />NONE';
...
$smarty->display(CURRENT_TEMPLATE_MODULE . 'popup_coupon_help'.HTML_EXT);
