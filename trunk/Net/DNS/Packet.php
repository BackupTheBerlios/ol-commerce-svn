...
class Net_DNS_Packet
{
    ...
    function parse($data)
    {
        ...
        if ($this->debug) {
            echo NEW_LINE;
            $section = ($this->header->opcode  == 'SQL_UPDATE') ? 'ZONE' : 'QUESTION';
        ...
        if ($this->debug) {
            echo NEW_LINE;
            $section = ($this->header->opcode == 'SQL_UPDATE') ? 'PREREQUISITE' :'ANSWER';
        ...
        /*
         *  Print and parse the SQL_UPDATE or AUTHORITY section of the packet
         */
        if ($this->debug) {
            echo NEW_LINE;
            $section = ($this->header->opcode == 'SQL_UPDATE') ? 'SQL_UPDATE' : 'AUTHORITY';
            echo ";; $section SECTION (" .
