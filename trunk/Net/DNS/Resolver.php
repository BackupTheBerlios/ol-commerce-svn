...
class Net_DNS_Resolver
{
    ...
    function string()
    {
        ...
        $state .= ';;  domain       = ' . $this->domain . NEW_LINE;
        $state .= ';;  searchlist   = ' . implode(' ', $this->searchlist) . NEW_LINE;
        $state .= ';;  nameservers  = ' . implode(' ', $this->nameservers) . NEW_LINE;
        $state .= ';;  port         = ' . $this->port . NEW_LINE;
        $state .= ';;  tcp_timeout  = ';
        $state .= ($this->tcp_timeout ? $this->tcp_timeout : 'indefinite') . NEW_LINE;
        $state .= ';;  retrans  = ' . $this->retrans . '  ';
        $state .= 'retry    = ' . $this->retry . NEW_LINE;
        $state .= ';;  usevc    = ' . $this->usevc . '  ';
        $state .= 'stayopen = ' . $this->stayopen . '    ';
        $state .= 'igntc = ' . $this->igntc . NEW_LINE;
        $state .= ';;  defnames = ' . $this->defnames . '  ';
        $state .= 'dnsrch   = ' . $this->dnsrch . NEW_LINE;
        $state .= ';;  recurse  = ' . $this->recurse . '  ';
        $state .= 'debug    = ' . $this->debug . NEW_LINE;
        return $state;
    }
            ...
            foreach ($sock as $k => $s) {
                ...
                if ($changed) {
                    ...
                    if ($buf === false) {
                        ...
                        if ($this->debug) {
                            echo ';; no data could be read from ' . $peerhost[$k] . ':' . $peerport[$k] . NEW_LINE;
                            echo ';; socket_error: ' . socket_strerror(socket_last_error()) . NEW_LINE;
                        }
