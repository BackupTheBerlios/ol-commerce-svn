<?php
/*------------------------------------------------------------------------------
   $Id: affiliate_details.php,v 1.1.1.1.2.1 2007/04/08 07:16:04 gswkaiser Exp $
...
if (isset($_POST['action'])) {
    ...
    $check_email_query = olc_db_query("select count(*) as total from " . TABLE_AFFILIATE . " where affiliate_email_address = '" .  olc_db_input($a_email_address) . "' and affiliate_id != '" . olc_db_input($_SESSION['affiliate_id']) . APOS);
    ...
    if (!$error) {
		...
		olc_db_perform(TABLE_AFFILIATE, $sql_data_array, 'update', "affiliate_id = '" . olc_db_input($_SESSION['affiliate_id']) . APOS);
		
        olc_redirect(olc_href_link(FILENAME_AFFILIATE_DETAILS_OK, '', SSL));
    }
}

$breadcrumb->add(NAVBAR_TITLE, olc_href_link(FILENAME_AFFILIATE, '', SSL));
$breadcrumb->add(NAVBAR_TITLE_DETAILS, olc_href_link(FILENAME_AFFILIATE_DETAILS, '', SSL));

require(DIR_WS_INCLUDES . 'header.php');

$affiliate_query = olc_db_query("select * from " . TABLE_AFFILIATE . " where affiliate_id = '" . $_SESSION['affiliate_id'] . APOS);
$affiliate = olc_db_fetch_array($affiliate_query);
$smarty->assign('affiliate', $affiliate);

//W. Kaiser - AJAX
$smarty->assign('FORM_ACTION', olc_draw_form('affiliate_details', olc_href_link(FILENAME_AFFILIATE_DETAILS, '', SSL), 'post', 'onsubmit="return check_form(\'affiliate_details\');"'));
//W. Kaiser - AJAX
$smarty->assign('HIDDEN_ACTION', olc_draw_hidden_field('action', 'process'));
$smarty->assign('IMAGE_TABLE_BACKGROUND', olc_image(DIR_WS_IMAGES . 'table_background_account.gif', HEADING_TITLE, HEADING_IMAGE_WIDTH, HEADING_IMAGE_HEIGHT));
...
$main_content=$smarty->fetch(CURRENT_TEMPLATE_MODULE . 'affiliate_details'.HTML_EXT,SMARTY_CACHE_ID);
