<?php
/*
  $Id: IPN.class.php,v 1.1.1.1.2.1 2007/04/08 07:18:08 gswkaiser Exp $
...
require_once(PAYPAL_IPN_DIR.'Classes/Client/Connector.class.php');
class PayPal_IPN extends PayPal_Client_Connector {
  ...
  function insert($paypal_id = '') {
    ...
    olc_db_perform(TABLE_PAYPAL, $sql_data_array);
    $this->ipnID = olc_db_insert_id();
  ...
  function updatePaymentStatusHistory($paypal_id) {
    ...
    $sql_data_array['date_added'] = 'now()';
    olc_db_perform(TABLE_PAYPAL_PAYMENT_STATUS_HISTORY, $sql_data_array);
  }
  ...
  function updateStatus($paypal_id) {
    ...
    olc_db_perform(TABLE_PAYPAL, $sql_data_array, 'update', "paypal_id = '" . (int)$paypal_id . APOS);
  ...
  function updateOrderStatus($paypal_id,$status) {
    ...
    olc_db_perform(TABLE_ORDERS, $sql_data_array, 'update', "payment_id = '" . (int)$paypal_id . APOS);

    //Orders Status History
    $sql_query = olc_db_query("select orders_id from " . TABLE_ORDERS . " where payment_id = '" . (int)$paypal_id . APOS);
    $sql_result = olc_db_fetch_array($sql_query);
    $sql_data_array = array(
                              'orders_id'     => $sql_result['orders_id'],
                              'orders_status_id' => olc_db_input($status),
                              'date_added'    => 'now()'
                            );
    olc_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array, 'insert');
  }

  function queryTxnID($txn_id) {
    $sql_query = olc_db_query("select paypal_id from " . TABLE_PAYPAL . " where txn_id = '" . olc_db_input($txn_id) . "' limit 0,1");
    return olc_db_fetch_array($sql_query);
  }

  function queryPendingStatus($txn_id) {
    $sql_query = olc_db_query("select paypal_id, payment_status, pending_reason from " . TABLE_PAYPAL . " where txn_id = '" . olc_db_input($txn_id) . "' limit 0,1");
    return olc_db_fetch_array($sql_query);
  }
  ...
  function uniqueTxnID() {
    ...
    if (isset($this->uniqueTxnID)) {
      ...
      $txn_id_query = olc_db_query("select txn_id from " . TABLE_PAYPAL . " where txn_id = '" . olc_db_input($this->txnID()) . "' limit 0,1");
      if (!olc_db_num_rows($txn_id_query)) { //txn_id doesn't exist
  ...
  function isAuction() {
    return (isset($this->key['for_auction']) && $this->key['for_auction'] === TRUE_STRING_S);
  }
    ...
    if(defined('TABLE_PAYPAL_AUCTION')) {
      if($this->isAuction() && strlen($this->key['item_number']) > 0) {
        ...
        foreach($items as $key => $item_id) {
          ...
          $txn_check = olc_db_query("select auction_buyer_id from " . TABLE_PAYPAL_AUCTION . " where paypal_id = '" . (int)$paypal_id . "' and item_number = '" . olc_db_input($item_id) . APOS);
          if (!olc_db_num_rows($txn_check)) {
            ...
            olc_db_perform(TABLE_PAYPAL_AUCTION, $sql_data_array);
          }
         }
       }
    }
  ...
  function uniqueTxnSignature() {
    ...
    if (isset($this->uniqueTxnSignature)) {
      ...
      $txn_sign_query = olc_db_query("select txn_sign from " . TABLE_PAYPAL . " where txn_sign = '" . olc_db_input($this->txnSignature()) . "' limit 0,1");
      if (!olc_db_num_rows($txn_sign_query)) { //txn_sign doesn't exist
