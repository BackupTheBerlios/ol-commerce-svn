<?php
/*
  $Id: TransactionDetails.class.php,v 1.1.1.1 2006/12/22 13:43:30 gswkaiser Exp $
...
class PayPal_TransactionDetails {
  ...
  function date($raw_date) {
  	...
  	{
	    $cal = array('01' => 'Jan', '02' => 'Feb', '03' => 'Mär', '04' => 'Apr', '05' => 'Mai',  '06' => 'Jun',  '07' => 'Jul', '08' => 'Aug', '09' => 'Sep', '10' => 'Okt', '11' => 'Nov', '12' => 'Dez');
  	}
	else
  	{
  ...
  function simpleQuery($paypal_id) {
    ...
    $ipn_query = olc_db_query("select " . implode(',',array_merge($info,$txn)) . " from " . $this->paymentTableName . " where paypal_id = '" . (int)$paypal_id . APOS);
  ...
  function transactionSignature($order_id) {
    $txn_signature_query = olc_db_query("select txn_signature from " . TABLE_ORDERS_SESSION_INFO . " where orders_id = '" . (int)$order_id . "' limit 1");
    if (olc_db_num_rows($txn_signature_query)) {
      $txn_signature = olc_db_fetch_array($txn_signature_query);
