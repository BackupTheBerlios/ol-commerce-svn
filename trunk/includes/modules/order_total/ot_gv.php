<?php
/* -----------------------------------------------------------------------------------------
   $Id: ot_gv.php,v 1.1.1.1.2.1 2007/04/08 07:18:04 gswkaiser Exp $
  ...
  class ot_gv {
    ...
    function ot_gv() {
      ...
      $this->checkbox =olc_draw_checkbox_field('c' . $this->code,EMPTY_STRING,
      false,'onclick="javascript:submitFunction()"').HTML_NBSP.$this->user_prompt;
			/*
...
//        }
        ...
        if ($od_amount > 0) {
          $this->output[] = array('title' => $this->title . ':',
                           'text' => '<b><font color="ff0000">-' .olc_format_price($od_amount, $price_special=1,
                           $calculate_currencies=false) . '</font></b>',
                           'value' => olc_format_price($od_amount, $price_special=0,
    ...
    {
      return $this->user_has_gv_account(CUSTOMER_ID);
    }
      ...
      if ($this->selection_test())
      {
        $output_string =  '
            <td nowrap="nowrap" class="main">
        			<b>' . $this->checkbox . HTML_B_END . HTML_NBSP. '
        		</td>
';
      }
...
//        $gv_order_amount += 0.001;
        ...
        if (MODULE_ORDER_TOTAL_GV_QUEUE == FALSE_STRING_S) {
          ...
          if ($customer_gv) {
            $gv_update=olc_db_query(SQL_UPDATE . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . $total_gv_amount .
            "' where customer_id = '" . CUSTOMER_ID . APOS);
          } else {
            $gv_insert=olc_db_query(INSERT_INTO.TABLE_COUPON_GV_CUSTOMER." (customer_id, amount) values ('".
            CUSTOMER_ID . "', '" . $total_gv_amount . "')");
          }
        } else {
         // GV_QUEUE is true - so queue the gv for release by store owner
          $gv_insert=olc_db_query(INSERT_INTO . TABLE_COUPON_GV_QUEUE .
          	" (customer_id, order_id, amount, date_created, ipaddr) values ('" . CUSTOMER_ID . "', '" .
          	$insert_id . "', '" . $gv_order_amount . "', NOW(), '" . $REMOTE_ADDR . "')");
        }
      }
    }
    ...
    function credit_selection() {
      global $currencies;
      $selection_string = '';
      $gv_query = olc_db_query("select coupon_id from " . TABLE_COUPONS .
      " where coupon_type = 'G' and coupon_active='Y'");
      /*
      if (olc_db_num_rows($gv_query)) {
        $selection_string .= '<tr>' . NEW_LINE;
        $selection_string .= '  <td width="10">' .  olc_draw_separator('pixel_trans.gif', '10', '1') .'</td>';
        $selection_string .= '  <td class="main">' . NEW_LINE;
        $selection_string .= TEXT_ENTER_GV_CODE . olc_draw_input_field('gv_redeem_code') . '</td>';
    ...
    function apply_credit() {
     ...
     if (isset($_SESSION['cot_gv']) && $_SESSION['cot_gv'] == true) {
        ...
        $gv_update = olc_db_query(SQL_UPDATE . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . $gv_amount .
        "' where customer_id = '" . CUSTOMER_ID . APOS);
      }
      return $gv_payment_amount;
    }
    ...
    function collect_posts() {
      ...
      if ($_POST['gv_redeem_code']) {
        $gv_query = olc_db_query("select coupon_id, coupon_type, coupon_amount from " . TABLE_COUPONS .
        " where coupon_code = '" . $_POST['gv_redeem_code'] . APOS);
        $gv_result = olc_db_fetch_array($gv_query);
        if (olc_db_num_rows($gv_query) != 0) {
          $redeem_query = olc_db_query("select * from " . TABLE_COUPON_REDEEM_TRACK .
          " where coupon_id = '" . $gv_result['coupon_id'] . APOS);
          if ( (olc_db_num_rows($redeem_query) != 0) && ($gv_result['coupon_type'] == 'G') ) {
            olc_redirect(olc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' .
            urlencode(ERROR_NO_INVALID_REDEEM_GV), SSL));
          }
        }
        if ($gv_result['coupon_type'] == 'G') {
          ...
          $gv_amount_query=olc_db_query("select amount from " . TABLE_COUPON_GV_CUSTOMER .
          " where customer_id = '" . CUSTOMER_ID . APOS);
          $customer_gv = false;
          ...
          $gv_update = olc_db_query(SQL_UPDATE . TABLE_COUPONS . " set coupon_active = 'N' where coupon_id = '" .
           $gv_result['coupon_id'] . APOS);
          $gv_redeem = olc_db_query("insert into  " . TABLE_COUPON_REDEEM_TRACK .
          " (coupon_id, customer_id, redeem_date, redeem_ip) values ('" . $gv_result['coupon_id'] . "', '" .
           CUSTOMER_ID . "', now(),'" . $REMOTE_ADDR . "')");
          if ($customer_gv) {
            // already has gv_amount so update
            $gv_update = olc_db_query(SQL_UPDATE . TABLE_COUPON_GV_CUSTOMER . " set amount = '" .
            $total_gv_amount . "' where customer_id = '" . CUSTOMER_ID . APOS);
          } else {
            // no gv_amount so insert
            $gv_insert = olc_db_query(INSERT_INTO . TABLE_COUPON_GV_CUSTOMER .
            " (customer_id, amount) values ('" . CUSTOMER_ID . "', '" . $total_gv_amount . "')");
          }
     ...
     if ($_POST['submit_redeem_x'] && $gv_result['coupon_type'] == 'G')
     	olc_redirect(olc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' .
     	urlencode(ERROR_NO_REDEEM_CODE), SSL));
   }
    ...
    function calculate_credit($amount) {
      ...
      return olc_round($gv_payment_amount,2);
    }
    ...
    function calculate_tax_deduction($amount, $od_amount, $method) {
      ...
      switch ($method) {
        ...
        break;
        case 'Credit Note':
          $tax_rate = olc_get_tax_rate($this->tax_class, $order->delivery['country']['id'],
           $order->delivery['zone_id']);
          $tax_desc = olc_get_tax_description($this->tax_class, $order->delivery['country']['id'],
           $order->delivery['zone_id']);
...
//          $order->info['total'] -= $tod_amount;   //// ????? Strider
    ...
    function install()
    {
    	$sql_insert=INSERT_INTO . TABLE_CONFIGURATION . " (configuration_id, configuration_key, configuration_value, configuration_group_id, sort_order, ";
    	$date_added="date_added) values ('', 'MODULE_ORDER_TOTAL_GV_";
    	$sql_set_date="set_function, ".$date_added;
    	$sql_insert_set_date=$sql_insert.$sql_set_date;
    	$select_option="olc_cfg_select_option(array(\'true\', \'false\'), ', now())";
      olc_db_query($sql_insert_set_date."STATUS', 'true', '6', '1','olc_cfg_select_option(array(\'true\', \'false\'), ', now())");
      olc_db_query($sql_insert.$date_added."SORT_ORDER', '80', '6', '2', now())");
      olc_db_query($sql_insert_set_date."QUEUE', 'true', '6', '3','".$select_option);
      olc_db_query($sql_insert_set_date."INC_SHIPPING', 'true', '6', '5', '".$select_option);
      olc_db_query($sql_insert_set_date."INC_TAX', 'true', '6', '6','".$select_option);
      olc_db_query($sql_insert_set_date."CALC_TAX', 'None', '6', '7','olc_cfg_select_option(array(\'None\', \'Standard\', \'Credit Note\'), ', now())");
      olc_db_query($sql_insert."use_function, ".$sql_set_date."TAX_CLASS', '0', '6', '0', 'olc_get_tax_class_title', 'olc_cfg_pull_down_tax_classes(', now())");
      olc_db_query($sql_insert_set_date."CREDIT_TAX', 'false', '6', '8','".$select_option);
    }
