<?php
/* -----------------------------------------------------------------------------------------
$Id: zones.php,v 1.1.1.1.2.1 2007/04/08 07:18:13 gswkaiser Exp $
 ...
 */
	function zones() {
		...
		$this->enabled = ((strtolower(MODULE_SHIPPING_ZONES_STATUS) == TRUE_STRING_S) ? true : false);
 ...
 */
	...
	*/
	function quote($method = '')
	{
		...
		if ($dest_zone == 0)
		{
			...
			for ($i=0; $i<$size; $i+=2) {
				if ($shipping_weight <= $zones_table[$i])
				{
					...
					$shipping_method = MODULE_SHIPPING_ZONES_TEXT_WAY . BLANK . $order_delivery_country['title'] . ', '.
					MODULE_SHIPPING_ZONES_TEXT_WEIGHT.': ' .
		...
		if ($this->tax_class > 0) {
			$this->quotes['tax'] =
			olc_get_tax_rate($this->tax_class, $order_delivery_country['id'], $order->delivery['zone_id']);
		}
	...
	function check() {
		if (!isset($this->_check)) {
			...
			" where configuration_key = 'MODULE_SHIPPING_ZONES_STATUS'");
	...
	function install()
	{
		$sql_insert0=INSERT_INTO . TABLE_CONFIGURATION .
			" (configuration_key, configuration_value, configuration_group_id, sort_order, ";
		$sql_insert1="date_added) values ('MODULE_SHIPPING_ZONES_";
		$sql_insert2="', '0', '6', '0', now())";
		$sql_insert=$sql_insert0 . $sql_insert1;

		olc_db_query($sql_insert0."set_function, ".$sql_insert1."STATUS'" .
			", 'true', '6', '0', 'olc_cfg_select_option(array(\'True\', \'False\'), ', now())");
		olc_db_query($sql_insert."ALLOWED".$sql_insert2);
		olc_db_query($sql_insert0."use_function, set_function, ".$sql_insert1."CLASS'".
			", '0', '6', '0', 'olc_get_tax_class_title', 'olc_cfg_pull_down_tax_classes(', now())");
		olc_db_query($sql_insert."SORT_ORDER".$sql_insert2);
		for ($i = 1; $i <= $this->num_zones; $i++) {
			if ($i == 1)
			{
			...
			olc_db_query($sql_insert."COUNTRIES_" . $i ."', '" . $default_countries .$sql_insert2);
			olc_db_query($sql_insert."COST_" . $i ."', '3:8.50,7:10.50,99:20.00'".$sql_insert2);
			olc_db_query($sql_insert."HANDLING_" . $i.APOS.$sql_insert2);
		}
	}
	...
	function keys() {
		$keys = array('MODULE_SHIPPING_ZONES_STATUS','MODULE_SHIPPING_ZONES_ALLOWED', 'MODULE_SHIPPING_ZONES_TAX_CLASS',
