<?php
/* -----------------------------------------------------------------------------------------
$Id: navigation_history.php,v 1.1.1.1.2.1 2007/04/08 07:17:47 gswkaiser Exp $
	...
	function add_current_page() {
		...
		for ($i=0, $n=sizeof($this->path); $i<$n; $i++)
		{
			if ( ($this->path[$i]['page'] == basename($PHP_SELF)) )
			{
				if (isset($cPath))
				{
					$this_path=$this->path[$i]['get']['cPath'];
					if (!isset($this_path)) {
						continue;
					} else {
						if ($this_path == $cPath) {
						...
						{
							$old_cPath = explode(UNDERSCORE, $this_path);
							$new_cPath = explode(UNDERSCORE, $cPath);
