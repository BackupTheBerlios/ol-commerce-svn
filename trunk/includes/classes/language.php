<?php
/* --------------------------------------------------------------
$Id: language.php,v 1.1.1.1.2.1 2007/04/08 07:17:47 gswkaiser Exp $
...
class language {
	...
	function language($lng = EMPTY_STRING)
	{
		...
		if (function_exists('olc_db_query'))
		{
			$languages_query = olc_db_query("select languages_id, name, code, image, directory,language_charset from " .
			...
			while ($languages = olc_db_fetch_array($languages_query)) {
				...
				'charset' => $languages['language_charset']);
			}
			$this->browser_languages = EMPTY_STRING;
	...
	function get_browser_language() {
		$this->browser_languages = explode(COMMA, getenv('HTTP_ACCEPT_LANGUAGE'));

		for ($i=0, $n=sizeof($this->browser_languages); $i<$n; $i++) {
			reset($this->languages);
			while (list($key, $value) = each($this->languages)) {
				if (eregi('^(' . $value[0] . ')(;q=[0-9]\\.[0-9])?$', $this->browser_languages[$i]) &&
