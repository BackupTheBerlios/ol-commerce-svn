...
/* -----------------------------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------------------------
   based on: 
   (c) 2005 olc_validate_vatid_status.inc.php 899 2005-04-29
...
class vat_validation {
	...
	function getInfo($vat_id = '', $customers_id = '', $customers_status = '', $country_id = '', $guest = false) {
		...
		if ($customers_id) {
			$customers_status_query = olc_db_query("SELECT customers_status FROM ".TABLE_CUSTOMERS." WHERE customers_id = '".$customers_id."'");
	...
	function validate_vatid($vat_id, $country_id) {
		...
		$country_array = array ();
		$country_array = olc_get_countriesList($country_id, true);
