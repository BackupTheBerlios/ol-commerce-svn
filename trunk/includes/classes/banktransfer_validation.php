...
version 1.3.6   2005/07/27

First based on:
osCommerce, Open Source E-Commerce Solutions
http://www.oscommerce.com

/includes/modules/banktransfer_validation.php
...
New methods 2005: Frank Maroke <info@fmce.de>

Ported to OL-Commerce by W. Kaiser (w.kaiser@fortune.de) 2006-01-12
...
class AccountCheck {
	...
	//      $blz_query = olc_db_query("SELECT * from " . TABLE_GERMANBT_BLZ . " WHERE blz = '" . $blz . APOS);
	//    } else {
	//      $blz_query = olc_db_query("SELECT * from " . TABLE_BANKTRANSFER . " WHERE blz = '" . $blz . APOS);
	//    }
	//    if (olc_db_num_rows($blz_query)){
	//      $data = olc_db_fetch_array ($blz_query);
	//    }else
	//      $data = -1;
	//    return $data;
	//  }
	...
	// Diese function gibt die Bankinformationen aus der Datenbank zurück*/
	//  function query($blz) {
	//    if (defined(MODULE_PAYMENT_BANKTRANSFER_DATABASE_BLZ) && MODULE_PAYMENT_BANKTRANSFER_DATABASE_BLZ == TRUE_STRING_S)
	...
	*/
	...
	function Mark87($AccountNo) {
		...
		if (substr($AccountNo,2,1) == '9') {
			...
			// print_r($AccountNoTemp); echo HTML_BR;
			$i = 4;
	...
	/* --- No BLZ assigned to this method at the moment (list june 2005) ---*/
	...
	/* --- Die Funktionen B6 bis B8 werden zum 05.09.2005 benoetigt! --- */
	...
	//	W. Kaiser - AJAX
	//	function CheckAccount($banktransfer_number, $banktransfer_blz, $banktransfer_bankname)
	function CheckAccount($banktransfer_number, $banktransfer_blz, $banktransfer_bankname, $banktransfer_prz)
	//	W. Kaiser - AJAX
	{
		//	W. Kaiser - AJAX
		$KontoNR = ereg_replace('[^0-9]', '', $banktransfer_number);
		$BLZ = ereg_replace('[^0-9]', '', $banktransfer_blz);
		$PRZ = str_pad ($banktransfer_prz, 2, "0", STR_PAD_LEFT);

		/*     Beginn Implementierung */
		$this->Bankname = $banktransfer_bankname;
		$this->PRZ = $PRZ;
