...
class product {
	...
	function product($pID = 0) {
		...
		and pd.language_id = '".SESSION_LANGUAGES_ID.APOS;

		$product_query = olc_db_query($product_query);

		if (!olc_db_num_rows($product_query, true)) {
			$this->isProduct = false;
		} else {
			$this->isProduct = true;
			$this->data = olc_db_fetch_array($product_query, true);
		}
	...
	function getAttributesCount() {
		...
		olc_db_query("select count(*) as total from ".
		TABLE_PRODUCTS_OPTIONS." popt, ".
		TABLE_PRODUCTS_ATTRIBUTES." patrib
		where 
		patrib.products_id='".$this->pID."' and 
		patrib.options_id = popt.products_options_id and popt.language_id = '".SESSION_LANGUAGE_ID.APOS);
		$products_attributes = olc_db_fetch_array($products_attributes_query, true);
	...
	function getReviewsCount() {
		...
		TABLE_REVIEWS_DESCRIPTION." rd
			where 
			r.products_id = '".$this->pID."' and 
			r.reviews_id = rd.reviews_id and 
			rd.languages_id = '".SESSION_LANGUAGE_ID."' and rd.reviews_text !=EMPTY_STRING");
		$reviews = olc_db_fetch_array($reviews_query, true);
   ...
   order by reviews_id DESC");
	...
	function getAlsoPurchased() {
		...
		and pd.language_id = '".SESSION_LANGUAGES_ID."'
		and opb.products_id = pd.products_id
		".$group_check."
		".$fsk_lock."
		group by p.products_id order by o.date_purchased desc limit ".MAX_DISPLAY_ALSO_PURCHASED;
		$orders_query = olc_db_query($orders_query);
		while ($orders = olc_db_fetch_array($orders_query, true)) {
	...
	function getCrossSells() {
		...
		$cs_groups = olc_db_query($cs_groups);
		$cross_sell_data = array ();
		if (olc_db_num_rows($cs_groups, true)>0) {
			while ($cross_sells = olc_db_fetch_array($cs_groups, true)) {
				...
				$cross_query = olc_db_query($cross_query);
				if (olc_db_num_rows($cross_query, true) > 0)
				$cross_sell_data[$cross_sells['products_xsell_grp_name_id']] = array ('GROUP' => olc_get_cross_sell_name($cross_sells['products_xsell_grp_name_id']), 'PRODUCTS' => array ());

				while ($xsell = olc_db_fetch_array($cross_query, true)) {
	...
	function getReverseCrossSells() {
		...
		TABLE_PRODUCTS_DESCRIPTION." pd
		where 
		xp.xsell_id = '".$this->pID."' and 
		xp.products_id = p.products_id ".$fsk_lock.$group_check." and 
	...
	function getGraduated() {
		...
		personal_offer
		FROM".
		TABLE_PERSONAL_OFFERS_BY. CUSTOMER_STATUS_ID."
		WHERE
		products_id = '".$this->pID."'
		ORDER BY quantity ASC");
		
		$staffel = array ();
		while ($staffel_values = olc_db_fetch_array($staffel_query, true)) {
		...
		for ($i = 0, $n = sizeof($staffel); $i < $n; $i ++) {
			if ($staffel[$i]['stk'] == 1) {
				...
				if ($staffel[$i +1]['stk'] != EMPTY_STRING)
				$quantity = $staffel[$i]['stk'].DASH. ($staffel[$i +1]['stk'] - 1);
			} else {
				$quantity = ' > '.$staffel[$i]['stk'];
				if ($staffel[$i +1]['stk'] != EMPTY_STRING)
				$quantity = $staffel[$i]['stk'].DASH. ($staffel[$i +1]['stk'] - 1);
			...
			if ($product_info['products_vpe_status'] == 1 && $product_info['products_vpe_value'] != 0.0 && $staffel[$i]['price'] > 0) {
				...
				$vpe = $xtPrice->xtcFormat($vpe, true, $product_info['products_tax_class_id']).TXT_PER.olc_get_vpe_name($product_info['products_vpe']);
			}
	...
	function buildDataArray(&$array,$image='thumbnail') {
		...
		'PRODUCTS_LINK' => olc_href_link(FILENAME_PRODUCT_INFO,
		olc_product_link($array['products_id'], $array['products_name'])),
	...
	function productImage($name, $type) {
		if ($name == EMPTY_STRING) {
			if ($this->useStandardImage == 'true' && $this->standardImage != EMPTY_STRING)
